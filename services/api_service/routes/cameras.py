...
async def create(payload:CameraCreate,request:Request):return await svc(request).create(payload.model_dump())
@router.patch("/{camera_id}")
async def update(camera_id:str,payload:CameraUpdate,request:Request):return await svc(request).update(camera_id,payload.model_dump(exclude_unset=True))
@router.delete("/{camera_id}",status_code=204)
async def delete(camera_id:str,request:Request):await svc(request).delete(camera_id)
