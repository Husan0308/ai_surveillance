...
class CameraPersonTrackingFinal(_BaseTracking):
    ...
    def __init__(self) -> None:
        ...
        self.detector_target_hz = float(
            os.environ.get("CAMERA_V2_DETECT_TARGET_HZ", "3.0")
        )
        self.detector_min_hz = float(
            os.environ.get("CAMERA_V2_DETECT_MIN_HZ", "2.2")
        )
        self.detector_max_hz = float(
            os.environ.get("CAMERA_V2_DETECT_MAX_HZ", "3.6")
        )
    ...
    def _stabilize_tracker_config(path: Path) -> Path:
        ...
        if missing:
            raise RuntimeError(
                "Generated NvDCF config is missing required keys: "
                + ", ".join(missing)
            )
        path.write_text("\n".join(output) + "\n", encoding="utf-8")
        return path
    ...
    def _print_stats(self) -> bool:
        ...
        rates = {
            ...
            for cid, rows in self.detector_times.items()
        }
        rate_text = " ".join(
