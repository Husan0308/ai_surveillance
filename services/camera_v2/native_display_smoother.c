...
/*
 * Display-only bbox animation smoother.
 *
 * This function runs AFTER NvDCF + shadow promotion. It never changes tracker
 * state, detector metadata or object IDs; only NvDsObjectMeta.rect_params used by
 * nvdsosd are interpolated.
 *
 * Design:
 *   - center follows quickly (~1-2 frames) so walking boxes stay with the person;
 *   - width/height expand over ~3-5 frames, so raising an arm does not make the
 *     rectangle jump instantly;
 *   - shrinking is slower than expansion, keeping hands/head/feet inside during
 *     pose changes;
 *   - a tiny velocity lead offsets the visual latency introduced by smoothing;
 *   - same object_id across ACTIVE/SHADOW/fallback keeps one continuous animation.
 */
int camera_v2_smooth_display_boxes(uintptr_t buffer_ptr) {
    ...
    const float center_alpha = 0.78f;
    const float velocity_alpha = 0.50f;
    const float lead_frames = 0.22f;
    const float expand_alpha = 0.38f;
    const float shrink_alpha = 0.18f;
    ...
    for (NvDsMetaList *fnode = batch_meta->frame_meta_list; fnode != NULL; fnode = fnode->next) {
        ...
        for (NvDsMetaList *onode = frame_meta->obj_meta_list; onode != NULL; onode = onode->next) {
            ...
            float max_vx = width * 0.40f + 4.0f;
            float max_vy = height * 0.40f + 4.0f;
            ...
            for (uint64_t step = 0; step < delta_frames; ++step) {
                float dx = desired_cx - s->display_cx;
                float dy = desired_cy - s->display_cy;
                float max_step_x = width * 0.34f + 3.0f;
                float max_step_y = height * 0.34f + 3.0f;
                dx = clampf_smooth(dx, -max_step_x, max_step_x);
                dy = clampf_smooth(dy, -max_step_y, max_step_y);
                s->display_cx += dx * center_alpha;
                s->display_cy += dy * center_alpha;

                float alpha_w = width >= s->display_w ? expand_alpha : shrink_alpha;
                float alpha_h = height >= s->display_h ? expand_alpha : shrink_alpha;
                s->display_w += (width - s->display_w) * alpha_w;
                s->display_h += (height - s->display_h) * alpha_h;
            }
        ...
        /* Retire very old IDs so a recycled tracker ID starts cleanly later. */
        for (int i = 0; i < MAX_SMOOTH_STATES; ++i) {
            SmoothBoxState *s = &g_smooth_states[i];
            if (!s->valid || s->source_id != source_id) continue;
            if (frame_num > s->last_frame_num && frame_num - s->last_frame_num > 120) {
                s->valid = 0;
            }
        }
    }
