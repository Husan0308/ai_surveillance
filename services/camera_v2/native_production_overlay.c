...
static void style_unknown(NvDsObjectMeta *obj,
    ...
    char label[64];
    g_snprintf(label, sizeof(label), "Unknown_C%u_%02" G_GUINT64_FORMAT,
               source_id + 1, (guint64)track_id);
    ...
    obj->text_params.x_offset = (unsigned int)(obj->rect_params.left > 0.0f
                                                ? obj->rect_params.left : 0.0f);
    obj->text_params.y_offset = (unsigned int)(obj->rect_params.top >= 18.0f
                                                ? obj->rect_params.top - 18.0f
                                                : obj->rect_params.top);
...
int camera_v2_add_production_unknown(uintptr_t buffer_ptr,
                                     unsigned int source_id,
                                     const float *boxes,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        strncpy(obj->obj_label, "Unknown", MAX_LABEL_SIZE - 1);
        ...
        obj->tracker_bbox_info.org_bbox_coords = obj->detector_bbox_info.org_bbox_coords;
