...
"""Stage 6 diagnostic: Stage 5 plus exactly one nvdsosd element.

Graph:
    nvurisrcbin/NVDEC -> queue -> nvstreammux(batch=1)
    -> nvmultistreamtiler(1x1) -> nvvideoconvert
    -> video/x-raw(memory:NVMM),format=RGBA -> nvdsosd -> fakesink

No detector, tracker, display, EGL or Qt is present. The OSD has no object/text
metadata to draw; this stage proves only that nvdsosd can consume and release the
RGBA batched buffers on the target GPU/runtime.
"""
...
def main() -> int:
    ...
    requested_id = os.environ.get("CAMERA_V2_STAGE6_CAMERA", "CAM-01").strip()
    camera = next((c for c in settings.cameras if c.camera_id == requested_id), None)
    if camera is None:
        known = ", ".join(c.camera_id for c in settings.cameras)
        raise RuntimeError(f"unknown CAMERA_V2_STAGE6_CAMERA={requested_id!r}; known={known}")

    latency_ms = int(os.environ.get("CAMERA_V2_RTSP_LATENCY_MS", "250"))
    width = int(os.environ.get("CAMERA_V2_STAGE6_WIDTH", "2560"))
    height = int(os.environ.get("CAMERA_V2_STAGE6_HEIGHT", "1440"))
    gpu_id = int(settings.deepstream.gpu_id)

    pipeline = Gst.Pipeline.new("camera-stage6-osd")
    if pipeline is None:
        raise RuntimeError("could not create stage6 pipeline")
    ...
    source = make("nvurisrcbin", "stage6_source")
    queue = make("queue", "stage6_queue")
    mux = make("nvstreammux", "stage6_mux")
    tiler = make("nvmultistreamtiler", "stage6_tiler")
    convert = make("nvvideoconvert", "stage6_convert")
    capsfilter = make("capsfilter", "stage6_rgba_caps")
    osd = make("nvdsosd", "stage6_osd")
    sink = make("fakesink", "stage6_sink")
    ...
    # GPU OSD mode. Nothing is drawn in this diagnostic; metadata flow/buffer
    # lifetime is what we are testing.
    _set_if(osd, "process-mode", 1)
    _set_if(osd, "display-bbox", False)
    ...
    if qsrc is None:
        raise RuntimeError("stage6 queue src pad missing")
    if qsrc.link(mux_sink) != Gst.PadLinkReturn.OK:
        raise RuntimeError("stage6 queue -> nvstreammux sink_0 link failed")
    if not mux.link(tiler):
        raise RuntimeError("stage6 nvstreammux -> tiler link failed")
    if not tiler.link(convert):
        raise RuntimeError("stage6 tiler -> nvvideoconvert link failed")
    if not convert.link(capsfilter):
        raise RuntimeError("stage6 nvvideoconvert -> RGBA caps link failed")
    if not capsfilter.link(osd):
        raise RuntimeError("stage6 RGBA caps -> nvdsosd link failed")
    if not osd.link(sink):
        raise RuntimeError("stage6 nvdsosd -> fakesink link failed")
    ...
    def configure_rtsp_child(_bin, _sub_bin, element) -> None:
        ...
        print(
            f"STAGE6_RTSP camera={camera.camera_id} auth={'yes' if camera.username else 'no'} "
    ...
    def source_pad_added(_source, pad) -> None:
        ...
        print(f"STAGE6_LINK result={result} caps={caps_text}", flush=True)
    ...
    def on_bus(_bus, message) -> None:
        if message.type == Gst.MessageType.ERROR:
            ...
            print(
                f"STAGE6_ERROR source={src} message={err.message} debug={debug or ''}",
                file=sys.stderr,
                flush=True,
            )
            loop.quit()
        elif message.type == Gst.MessageType.EOS:
            print("STAGE6_EOS", flush=True)
            loop.quit()
        elif message.type == Gst.MessageType.STATE_CHANGED and message.src == pipeline:
            old, new, pending = message.parse_state_changed()
            print(
                f"STAGE6_STATE old={old.value_nick} new={new.value_nick} pending={pending.value_nick}",
                flush=True,
            )
    ...
    def stats_tick() -> bool:
        ...
        print(
            f"STAGE6_STATS camera={camera.camera_id} source_frames={counters['source']} "
    ...
    print(
        f"STAGE6_START camera={camera.camera_id} graph=nvurisrcbin/NVDEC->queue->nvstreammux(batch=1)->"
        f"nvmultistreamtiler(1x1,{width}x{height})->nvvideoconvert->NVMM-RGBA->nvdsosd->fakesink "
        "detector=0 tracker=0 display=0 qt=0",
        flush=True,
    )
    ...
    if result == Gst.StateChangeReturn.FAILURE:
        ...
        raise RuntimeError("stage6 pipeline refused PLAYING")
    ...
    print(
        f"STAGE6_STOP source_frames={counters['source']} mux_buffers={counters['mux']} "
        f"tiler_buffers={counters['tiler']} rgba_buffers={counters['rgba']} "
        f"osd_buffers={counters['osd']} sink_buffers={counters['sink']}",
        flush=True,
    )
    return 0 if counters["osd"] > 0 and counters["sink"] > 0 else 2
