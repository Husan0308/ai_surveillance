...
def load_settings(path: str | Path | None = None) -> Settings:
    ...
    return Settings(cameras=tuple(cameras), deepstream=DeepStreamConfig(gpu_id=int(ds.get("gpu_id", 0)), latency_ms=int(ds.get("latency_ms", 150)), drop_on_latency=_as_bool(ds.get("drop_on_latency", True)), reconnect_interval_sec=int(ds.get("reconnect_interval_sec", 2)), reconnect_attempts=int(ds.get("reconnect_attempts", -1)), decoder_extra_surfaces=int(ds.get("decoder_extra_surfaces", 4)), cudadec_memtype=int(ds.get("cudadec_memtype", 0)), udp_buffer_size=int(ds.get("udp_buffer_size", 1_048_576)), rtp_protocol=int(ds.get("rtp_protocol", 0)), batch_size=batch_size, mux_width=int(mux.get("width", 1280)), mux_height=int(mux.get("height", 720)), live_source=_as_bool(mux.get("live_source", True)), batched_push_timeout_us=int(mux.get("batched_push_timeout_us", 50_000)), sync_inputs=_as_bool(mux.get("sync_inputs", False)), display_enabled=_as_bool(display.get("enabled", False)), display_rows=int(display.get("rows", 2)), display_columns=int(display.get("columns", 3)), display_width=int(display.get("width", 1920)), display_height=int(display.get("height", 1080)), frame_transport=FrameTransportConfig(enabled=_as_bool(frame_transport.get("enabled", True)), directory=os.getenv("FRAME_BUS_DIR", str(frame_transport.get("directory", "/dev/shm/ai_surveillance"))), width=frame_width, height=frame_height, max_fps=frame_max_fps)))
