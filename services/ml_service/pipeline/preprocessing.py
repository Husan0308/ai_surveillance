...
class BatchPreprocessor:
    ...
    def prepare(self, batch: BatchOutput) -> PreparedBatch:
        ...
        for index, packet in enumerate(batch.frames):
            ...
            # One fused HWC/BGR -> CHW/RGB copy into reusable batch storage.
            cv2.mixChannels([resized], [packed[index, 0], packed[index, 1], packed[index, 2]],
                            [2, 0, 1, 1, 0, 2])
        ...
        return PreparedBatch(batch, packed, tuple(transforms),
                             (time.perf_counter() - started) * 1000, cpu_pack_ms)
