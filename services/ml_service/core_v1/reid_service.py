...
def _iou(a, b):
    ...
    bb = max(0.0, float(b.x2) - float(b.x1)) * max(0.0, float(b.y2) - float(b.y1))
    union = aa + bb - inter
    return inter / union if union > 0 else 0.0


def _center_distance(a, b):
    acx = (float(a.x1) + float(a.x2)) * 0.5
    acy = (float(a.y1) + float(a.y2)) * 0.5
    bcx = (float(b.x1) + float(b.x2)) * 0.5
    bcy = (float(b.y1) + float(b.y2)) * 0.5
    aw = max(1.0, float(a.x2) - float(a.x1))
    ah = max(1.0, float(a.y2) - float(a.y1))
    bw = max(1.0, float(b.x2) - float(b.x1))
    bh = max(1.0, float(b.y2) - float(b.y1))
...
class _OSNetExtractor:
    def __init__(self, config: dict):
        ...
        self.model_name = str(self.config.get("model", "osnet_x0_25"))
        self.model_path = str(self.config.get("model_path", "") or "")
        self.height = max(64, int(self.config.get("input_height", 256)))
        self.width = max(32, int(self.config.get("input_width", 128)))
        ...
        if self.device.startswith("cuda") and not use_cuda:
            raise RuntimeError("ReID CUDA requested but unavailable")

        try:
            torch.set_num_threads(max(1, int(self.config.get("torch_cpu_threads", 1))))
        except Exception:
            pass
        ...
        model = torchreid.models.build_model(
            ...
            num_classes=1000,
            loss="softmax",
            pretrained=not bool(self.model_path),
            use_gpu=use_cuda,
        )
        if self.model_path:
    ...
    def extract(self, crops):
        batch = self.torch.stack([self._tensor(crop) for crop in crops], dim=0)
...
class ReIDCoordinator:
    """Non-blocking, event-driven ReID beside the realtime camera hot path."""
    ...
    def __init__(self, frame_stores, detections, config: dict | None = None):
        ...
        self.selector = PersonCropSelector(self.config.get("crop"))
        self.identities = GlobalIdentityManager(self.config.get("identity"))

        self.poll_sec = max(0.01, float(self.config.get("poll_interval_ms", 35)) / 1000.0)
        self.track_timeout = max(0.5, float(self.config.get("local_track_timeout_sec", 2.0)))
        self.match_iou = max(0.0, float(self.config.get("local_match_iou", 0.12)))
        self.match_center = max(0.1, float(self.config.get("local_match_center", 0.72)))
        self.min_hits = max(1, int(self.config.get("min_track_hits", 2)))
        self.embed_cooldown = max(0.2, float(self.config.get("embed_cooldown_sec", 1.2)))
        self.refresh_sec = max(self.embed_cooldown, float(self.config.get("refresh_sec", 4.0)))
        self.quality_improvement = max(0.0, float(self.config.get("quality_improvement", 0.08)))
        self.batch_size = max(1, int(self.config.get("batch_size", 4)))
        self.batch_wait = max(0.0, float(self.config.get("batch_wait_ms", 20)) / 1000.0)
        self.max_job_age = max(0.1, float(self.config.get("max_job_age_ms", 700)) / 1000.0)

        self._lock = threading.Lock()
        self._stop = threading.Event()
        self._observer = None
        self._infer = None
        self._job_queue = queue.Queue(maxsize=max(1, int(self.config.get("queue_size", 8))))
        self._latest_jobs = {}
        self._last_result_frame = {}
        self._tracks = {cid: {} for cid in self.frame_stores}
        self._next_local = {cid: 1 for cid in self.frame_stores}
        ...
        self._submitted = 0
        self._embedded = 0
        self._replaced_jobs = 0
        self._stale_jobs = 0
        self._crop_rejects = 0
    ...
    def start(self):
        if not self.enabled:
            return
        self._stop.clear()
        self._observer = threading.Thread(target=self._observe_loop, name="core-v1-reid-observer", daemon=False)
        self._infer = threading.Thread(target=self._infer_loop, name="core-v1-reid-infer", daemon=False)
    ...
    def _associate(self, camera_id, boxes, observed_at):
        ...
        for tid, track in tracks.items():
            for di, box in enumerate(boxes):
                ...
                dist = _center_distance(track.box, box)
                if iou >= self.match_iou or dist <= self.match_center:
                    pairs.append(((1.0 - iou) + 0.25 * dist, tid, di))
        pairs.sort()

        used_t, used_d, assigned = set(), set(), []
        for _score, tid, di in pairs:
            ...
            used_t.add(tid)
            used_d.add(di)
            assigned.append((track, boxes[di]))

        for di, box in enumerate(boxes):
            if di in used_d:
                continue
            tid = self._next_local[camera_id]
            self._next_local[camera_id] += 1
            track = _LocalTrack(tid, box, observed_at)
            tracks[tid] = track
    ...
    def _observe_loop(self):
        while not self._stop.is_set():
            ...
            for camera_id, result in snapshot.items():
                if int(result.frame_id) <= int(self._last_result_frame.get(camera_id, -1)):
                    continue
                self._last_result_frame[camera_id] = int(result.frame_id)
                store = self.frame_stores.get(camera_id)
                frame = store.get_frame(result.frame_id) if store and hasattr(store, "get_frame") else None
                if frame is None:
                    self._frame_misses += 1
                    continue

                observed = float(result.frame_captured_monotonic)
                ...
                for track, box in assigned:
                    ...
                    if not decision.accepted:
                        self._crop_rejects += 1
                        continue
                    first = track.last_embed_at <= 0
                    due = now - track.last_embed_at >= self.refresh_sec
                    better = decision.score >= track.best_quality + self.quality_improvement
                    if not first and not due and not better:
                        continue
                    if not first and now - track.last_embed_at < self.embed_cooldown:
                        continue
                    track.last_embed_at = now
                    track.best_quality = max(track.best_quality, decision.score)
    ...
    def _pop_job(self, key):
        if key is None:
            return None
        with self._lock:
            return self._latest_jobs.pop(key, None)

    def _infer_loop(self):
        try:
            self._extractor = _OSNetExtractor(self.config)
            self._ready = True
            log.info(
                "CORE_V1_REID_READY model=%s device=%s",
                self.config.get("model", "osnet_x0_25"),
                self.config.get("device", "cpu"),
            )
        except Exception as exc:
            self._last_error = f"{type(exc).__name__}: {exc}"
            log.exception("CORE_V1_REID_DISABLED error=%s", self._last_error)
            return

        while not self._stop.is_set():
            try:
                key = self._job_queue.get(timeout=0.25)
            except queue.Empty:
                continue
            if key is None:
                break
            ...
            fresh_jobs = []
            for job in jobs:
                if now - job.observed_at > self.max_job_age:
                    self._stale_jobs += 1
                else:
            ...
            try:
                features = self._extractor.extract([job.crop for job in jobs])
            except Exception as exc:
                self._last_error = f"{type(exc).__name__}: {exc}"
                log.exception("CORE_V1_REID_BATCH_FAILED")
                continue
    ...
    def labels(self, camera_id: str):
        ...
        now = time.monotonic()
        for track in tracks:
            if track.global_id and now - track.last_seen <= self.track_timeout:
                result.append(
                    {
    ...
    def metrics(self):
        ...
        return {
            ...
            "ready": self._ready,
            "model": self.config.get("model", "osnet_x0_25"),
            "device": self.config.get("device", "cpu"),
            "submitted": self._submitted,
            "embedded": self._embedded,
            "embed_rate": self._embedded / elapsed,
            "batches": self._batches,
            "last_batch_ms": self._last_batch_ms,
            "replaced_jobs": self._replaced_jobs,
