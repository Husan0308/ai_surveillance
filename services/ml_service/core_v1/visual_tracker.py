...
class VisualTracker:
    ...
    def __init__(self, *, hold_ms=500, memory_ms=6000, prediction_ms=220,
        ...
        self.smoothing=min(0.99,max(0.05,float(smoothing)))
    ...
    def _excluded(self, box:VisualBox, source_width:float|None, source_height:float|None) -> bool:
        ...
        for x1,y1,x2,y2 in self.exclusion_zones:
            ...
            inter=_intersection(box_norm,zone)
            if inter/max(1e-9,_area(box_norm))>=self.exclusion_overlap_threshold:return True
        return False
    ...
    def _is_duplicate(self, box:VisualBox, other:VisualBox) -> bool:
        iou=_iou(box,other);containment=_containment(box,other);center_distance=_center_distance(box,other)
        return (iou>=self.duplicate_iou or (containment>=self.duplicate_containment and center_distance<=self.duplicate_center_distance) or self._fragment_duplicate_match(box,other))
    ...
    def update(self, result, now:float, source_width=None, source_height=None) -> None:
        ...
        for tid,track in self._tracks.items():
            ...
            for di,det in enumerate(detections):
                ...
                if iou>=self.match_iou or dist<=max_distance:
                    pairs.append((iou+(max(0.0,max_distance-dist)*0.30)-(0.08 if dormant else 0.0),tid,di))
        pairs.sort(reverse=True)

        matched_tracks=set()
        for _score,tid,di in pairs:
            if tid not in unmatched_tracks or di not in unmatched_dets:continue
            track=self._tracks[tid];det=detections[di];old=track.raw_box
            ...
            track.vx1=vb*track.vx1+va*nv[0];track.vy1=vb*track.vy1+va*nv[1]
            track.vx2=vb*track.vx2+va*nv[2];track.vy2=vb*track.vy2+va*nv[3]
            a=self.smoothing;b=1.0-a
            track.box=VisualBox(b*track.box.x1+a*det.x1,b*track.box.y1+a*det.y1,b*track.box.x2+a*det.x2,b*track.box.y2+a*det.y2,max(det.confidence,track.box.confidence*0.82))
            track.raw_box=det;track.last_seen=now;track.last_update=now;track.last_observation=observation;track.hits+=1
            matched_tracks.add(tid);unmatched_tracks.remove(tid);unmatched_dets.remove(di)
        ...
        for di in unmatched_dets:
            ...
            self._tracks[tid]=_Track(track_id=tid,box=det,raw_box=det,last_seen=now,last_update=now,last_observation=observation)
