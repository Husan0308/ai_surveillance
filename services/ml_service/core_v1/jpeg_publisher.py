...
class LatestJpegPublisher:
    ...
    def start(self):
        self._thread = threading.Thread(
            target=self._run,
            name=f"core-v1-jpeg-{self.camera_id}",
            daemon=False,
        )
        self._thread.start()
    ...
    def _draw_detection(self, image, source_width, source_height, now, display_frame_time):
        ...
        for box in boxes:
            x1 = max(0, min(w - 1, int(round(box.x1 * sx))))
            y1 = max(0, min(h - 1, int(round(box.y1 * sy))))
            x2 = max(0, min(w - 1, int(round(box.x2 * sx))))
            y2 = max(0, min(h - 1, int(round(box.y2 * sy))))
            if x2 <= x1 or y2 <= y1:
                continue
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            image = self._draw_detection(
                image, source_w, source_h, now, frame.captured_monotonic
            )
