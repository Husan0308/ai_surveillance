...
class MmapFramePublisher(LatestJpegPublisher):
    """Event-driven latest-only publisher without JPEG or HTTP in the hot path."""
    ...
    def _run(self):
        ...
        try:
            while not self._stop.is_set():
                ...
                # Presentation is capped, but there is no backlog: if a newer
                # camera frame arrives while waiting, replace the old one.
                ...
                with self._lock:
                    ...
                    # Keep the historical metric name for dashboards/tests; in
                    # mmap mode it means published frames, not JPEG encodes.
                    self.encoded += 1
