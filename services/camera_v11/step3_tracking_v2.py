...
class V11Step3TrackingV2(V11Step2ProductionFP32V18):
    """Frozen Step2 V25 detector plus CPU-only per-camera local tracking."""

    def __init__(self) -> None:
        super().__init__()
        if self.mode != "full":
            raise RuntimeError("Step3 V2 supports V11_STEP2_MODE=full only")
        camera_ids = [camera.camera_id for camera in self.cameras]
        self.tracker = V11PerCameraTrackerV2(camera_ids)
        ...
        print(
            ...
            "tracker=observation-centric-sparse-bytetrack scope=per-camera cpu=1 "
            "appearance=0 reid=0 face=0 tracker_gpu=0 frame_queue=0 "
            "display_topology_changed=0 detector_schedule_changed=0",
            flush=True,
        )
        print(
            "CAMERA_V11_STEP3_V2_POLICY low=0.18 high=0.30 new=0.30 match=0.22 "
            "low_match=0.18 reacquire=0.12 low_recovery=0.10 low_recovery_sec=1.8 "
            "confirm_hits=2 shadow=0.9s max_lost=3.0s lost_velocity_half_life=0.9s "
            "clock=capture-monotonic",
            flush=True,
        )
    ...
    def run(self) -> int:
        ...
        try:
            while not self.stop_requested:
                ...
                # Tracking consumes detector metadata only. It does not touch the display
                # path, GPU, or GStreamer frame ownership.
