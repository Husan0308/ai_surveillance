...
def _rank_stats(
    ...
    """Return deterministic best index/value plus second and margin in one sort."""

    ordered = sorted(values, key=lambda item: (-float(item[0]), int(item[1])))
    best_value, best_index = ordered[0]
    if len(ordered) < 2:
        return int(best_index), float(best_value), None, None
    second = float(ordered[1][0])
    return int(best_index), float(best_value), second, float(best_value - second)
...
def _assign_eligible(
    ...
    """Assign reciprocal eligible edges with a zero-overhead one-to-one fast path.

    Reciprocal row/column-best qualification already makes eligible edges
    structurally one-to-one.  In that normal case Hungarian cannot change the
    solution, so the edges are accepted directly.  A compact Hungarian fallback
    remains for defensive correctness if future qualification semantics ever
    produce competing eligible endpoints.
    """

    if not eligible:
        return set()
    rows = {row for row, _column in eligible}
    columns = {column for _row, column in eligible}
    if len(rows) == len(eligible) and len(columns) == len(eligible):
        return set(eligible)

    compact_rows = sorted(rows)
    compact_columns = sorted(columns)
    row_position = {value: index for index, value in enumerate(compact_rows)}
    column_position = {value: index for index, value in enumerate(compact_columns)}
    sentinel = -1.0e12
    weights = np.full(
        (len(compact_rows), len(compact_columns)), sentinel, dtype=np.float64
    )
    for row_index, column_index in eligible:
        weights[row_position[row_index], column_position[column_index]] = float(
            scores[(row_index, column_index)].robust_score
        )
    assignment_rows, assignment_columns = linear_sum_assignment(weights, maximize=True)
    assigned: set[tuple[int, int]] = set()
    for compact_row, compact_column in zip(
        assignment_rows.tolist(), assignment_columns.tolist(), strict=True
    ):
        key = (
            compact_rows[int(compact_row)],
            compact_columns[int(compact_column)],
        )
        if key in eligible:
            assigned.add(key)
    return assigned
...
def match_same_room_camera_pair_v1(
    ...
    *,
    ...
    for row_index, row in enumerate(left):
        row_stale = current_ns - int(row.last_seen_ns) > recent_ns
        for column_index, column in enumerate(right):
            key = (row_index, column_index)
            if row_stale or current_ns - int(column.last_seen_ns) > recent_ns:
                structural[key] = STALE
                scores[key] = GalleryPairScoreV1(
                    status="INVALID",
                    pair_count=0,
                    reason="track exceeds recent-age timeout",
                )
                continue
            ...
            if (
                ...
                and math.isfinite(score.robust_score)
            ):
    ...
    # Canonical track ordering supplies deterministic tie breaks.  Best, second
    # and margin are derived from a single ordering per row/column instead of the
    # previous duplicate sorts.
    ...
    for row_index, values in valid_values_by_row.items():
        best_index, best_value, second, margin = _rank_stats(values)
        row_best_index[row_index] = best_index
    ...
    for column_index, values in valid_values_by_column.items():
        best_index, best_value, second, margin = _rank_stats(values)
        column_best_index[column_index] = best_index
    ...
    for key, state in structural.items():
        ...
        if not reciprocal:
            pre_status[key] = (
                NON_RECIPROCAL,
                "not reciprocal row/column best",
                False,
            )
        elif settings.min_robust_score is not None and score < settings.min_robust_score:
            pre_status[key] = (
                LOW_SCORE,
                "robust score below configured diagnostic threshold",
                True,
            )
        elif (
            ...
            pre_status[key] = (
                LOW_MARGIN,
                "row or column margin below configured diagnostic threshold",
                True,
            )
        else:
            eligible.add(key)
            pre_status[key] = (
                ASSIGNMENT_CONFLICT,
                "eligible but not selected by assignment",
                True,
            )

    assigned = _assign_eligible(eligible, scores)

    diagnostics: list[SameRoomPairDiagnosticV1] = []
    append_diagnostic = diagnostics.append
    for row_index, row in enumerate(left):
