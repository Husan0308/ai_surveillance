...
#             for row in sy.sm.db.get_analytics_hourly(today_str):
...
class EventsPage(Page):
    ...
    def _load_events_for_date(self, date_str):
        """EventsService dan shu sanadagi eventlarni lokal ro'yxatga yuklash"""
        try:
            sm = self.hub.sys.sm
            if hasattr(sm, 'events_service') and sm.events_service:
                sm.events_service.load_by_date(date_str)
                self._events = [self._normalize_event(x) for x in sm.events_service.events]
            else:
                self._events = []
        except Exception as ex:
            print(f"[Events] ⚠ load error: {ex}", flush=True)
            self._events = []
        # Fallback: EventsService bo'sh bo'lsa, hub.sys.events dan ol (sana filtri rebuild da)
        if not self._events:
            self._events = [self._normalize_event(x) for x in getattr(self.hub.sys, 'events', [])]
        print(f"[Events] 📥 {date_str} → {len(self._events)} events", flush=True)
        ...
        try:
            ...
            for row_data in rows:
                ...
                if rec.last_seen is None:
                    status_text, status_color = "—", TH.FAINT
                else:
                    secs = (datetime.now() - rec.last_seen).total_seconds()
                    if secs < 60:
...
class ProfileDialog(QDialog):
    def __init__(self, rec, sm=None):
        ...
        if self.sm and hasattr(rec, 'db_id') and rec.db_id:
            try:
                ...
                for vis in visits:
                    cam = vis.get("camera_id", "?"); entered = vis.get("entered_at", "?")
                    dur = vis.get("duration_sec", 0) or 0; dur_min = dur // 60
                    left = vis.get("left_at", "")
                    line = f"🎥 {cam}   ·   {str(entered)[:16]}   ·   {dur_min} min"
                    if left: line += f"   → {str(left)[:16]}"
                    l = QLabel(line); l.setStyleSheet(f"color:{TH.DIM}; font-size:10.5px;")
                    l.setWordWrap(True); vv.addWidget(l)
                visits_loaded = len(visits) > 0
            except Exception as e: print(f"[Profile] visits error: {e}", flush=True)
        ...
        if self.sm and hasattr(rec, 'db_id') and rec.db_id:
            try:
                ...
                for evt in events:
                    t = evt.get("time", "?"); typ = evt.get("type", "?"); cam = evt.get("camera_id", "?")
                    conf = evt.get("confidence", 0); level = evt.get("level", "info")
                    icon = "🔔" if level == "warn" else "✅" if level == "ok" else "ℹ"
                    ...
                    l.setWordWrap(True); ev.addWidget(l)
                events_loaded = len(events) > 0
            except Exception as e: print(f"[Profile] events error: {e}", flush=True)
        ...
        timeline_data = [0] * 24
        try:
            pid = getattr(rec, 'db_id', None) or getattr(rec, 'id', None)
            if pid and self.sm and hasattr(self.sm, 'person_service'):
                rt = self.sm.person_service._get_realtime_timeline(pid)
                if rt and len(rt) == 24: timeline_data = rt
        except Exception as e: print(f"[Profile] timeline error: {e}", flush=True)
    ...
    def _edit_person(self):
        ...
        sm = self.sm
        if not sm or not hasattr(self.rec, 'db_id') or not self.rec.db_id: return
        ...
        try:
            with sm.db.lock:
                sm.db.conn.execute("UPDATE persons SET name=?, department=?, updated_at=? WHERE id=?",
                    (new_name.strip(), new_dept.strip(), datetime.now().isoformat(), self.rec.db_id))
                sm.db.conn.commit()
            self.accept()
        except Exception as e: print(f"[Edit] ERROR: {e}", flush=True)
    ...
    def _delete_person(self):
        ...
        try:
            sm.db.delete_person(self.rec.db_id); self.accept()
        except Exception as e: print(f"[Delete] ERROR: {e}", flush=True)
