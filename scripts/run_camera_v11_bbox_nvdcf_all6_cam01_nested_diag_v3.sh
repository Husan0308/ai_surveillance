...
LOG="${V11_BBOX_NVDCF_LOG:-/tmp/CAMERA_V11_BBOX_NVDCF_ALL6_CAM01_NESTED_DIAG_V3.log}"
APP_PY="${V11_DS_YOLO_PYTHON:-$HOME/ai_surveillance/.venv/bin/python}"
[[ -x "$APP_PY" ]] || APP_PY="$(command -v python3)"
fail(){ printf 'V11_BBOX_NVDCF_ALL6_CAM01_NESTED_DIAG_V3_PREFLIGHT RESULT=FAIL reason=%s\n' "$*" >&2; exit 1; }
...
"$APP_PY" -c 'import services.camera_v11.deepstream_trt86_nvdcf_bbox_cam01_nested_diag_v3' || fail "runtime_import_failed"
...
printf 'V11_BBOX_NVDCF_ALL6_CAM01_NESTED_DIAG_V3_PREFLIGHT RESULT=PASS branch=%s cameras=CAM-01,CAM-02,CAM-03,CAM-04,CAM-05,CAM-06 rtsp_sources=6 detector_hz=%s detector_conf=%s cam01_tracker=%s diag=conf,max_iou,best_track,nested,new_candidate shadow_min_conf=%s trt=%s log=%s\n' "$BRANCH_EXPECTED" "$V11_DS_YOLO_HZ" "$V11_DS_YOLO_CONF" "$CAM01_NESTED_CFG" "$V11_BBOX_SHADOW_DISPLAY_MIN_CONF" "$TRT_VERSION" "$LOG"
: >"$LOG"
exec > >(trap '' INT TERM; tee "$LOG") 2>&1
exec "$APP_PY" -u -m services.camera_v11.deepstream_trt86_nvdcf_bbox_cam01_nested_diag_v3
