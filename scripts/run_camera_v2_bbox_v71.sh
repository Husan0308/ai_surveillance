...
# V7.1 visibility hysteresis. A missing normal NvDCF output is allowed to reuse only
# the last real box for 320 ms. The old timestamp is never refreshed, so this cannot
# become an indefinitely frozen ghost box and it does not predict motion.
export CAMERA_V2_DISPLAY_EMPTY_HOLD_MS="${CAMERA_V2_DISPLAY_EMPTY_HOLD_MS:-320}"
export CAMERA_V2_DISPLAY_TRACK_MAX_AGE_MS="${CAMERA_V2_DISPLAY_TRACK_MAX_AGE_MS:-360}"
...
# Keep detector correction cadence unchanged from V7. The GPU-lane ordering is what
# changes: frame capture happens first, and only TRT inference owns the serialized lock.
...
fail() { printf 'CAMERA_BBOX_V71_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
echo "CAMERA_BBOX_V71_GPU ${GPU_LINE:-unknown}"
...
if [[ "$CAMERA_V2_DETECT_ENABLED" == "1" ]]; then
  ...
  if [[ ! -s "$CAMERA_V2_TRT86_ENGINE" && -f "$RESTORE_HELPER" ]]; then
    echo "CAMERA_BBOX_V71_ENGINE recovery=stash/local-search" >&2
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(f"CAMERA_BBOX_V71_PREFLIGHT ERROR: TensorRT 8.6.1 required, got {trt.__version__}")
print(f"CAMERA_BBOX_V71_TRT_ENV python={sys.executable} trt={trt.__version__} numpy={np.__version__}", flush=True)
PY
fi
...
from services.camera_v2.runtime_bbox_v71 import NvDCFNoFlickerRuntime  # noqa: F401
PY
  then MAIN_PYTHON="$candidate"; break; fi
done
[[ -n "$MAIN_PYTHON" ]] || fail "no Python can import V7.1 no-flicker runtime"

export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"
printf '%s\n' \
  "CAMERA_BBOX_V71_PREFLIGHT status=OK python=$MAIN_PYTHON" \
  "CAMERA_BBOX_V71_PROFILE source=${CAMERA_V2_SOURCE_FPS}fps tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz detector=672x384@${CAMERA_V2_DETECT_HZ}Hz/cam" \
  "CAMERA_BBOX_V71_POLICY last_good_hold=${CAMERA_V2_DISPLAY_EMPTY_HOLD_MS}ms max_age=${CAMERA_V2_DISPLAY_TRACK_MAX_AGE_MS}ms prediction=0 shadow_render=0 capture_before_gpu_lane=1 gpu_lane=trt-only" \
  "CAMERA_BBOX_V71_PIPELINE decode-once->tee->{display,tracker/NvDCF,detector/TRT86} display_never_waits_for_analytics=1"

restart_count=0
while true; do
  set +e
  "$MAIN_PYTHON" -u -m services.camera_v2.runtime_bbox_v71
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_BBOX_V71_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
