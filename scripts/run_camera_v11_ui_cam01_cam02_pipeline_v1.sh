...
cd "$(dirname "$0")/.."
ROOT="$PWD"
BRANCH_EXPECTED="rebuild/service-architecture-v11-ui-sentinel-cam01-cam02-v1-20260902"
LOG="${V11_DS_YOLO_LOG:-/tmp/CAMERA_V11_DS_YOLO_UI_CAM01_CAM02.log}"
APP_PY="${V11_DS_YOLO_PYTHON:-$HOME/ai_surveillance/.venv/bin/python}"
[[ -x "$APP_PY" ]] || APP_PY="$(command -v python3)"
fail(){ printf 'V11_UI_CAM01_CAM02_PIPELINE_PREFLIGHT RESULT=FAIL reason=%s\n' "$*" >&2; exit 1; }
...
"$APP_PY" -c 'import services.camera_v11.deepstream_trt86_multi_ui_cam01_cam02_v1' || fail "runtime_import_failed"
printf 'V11_UI_CAM01_CAM02_PIPELINE_PREFLIGHT RESULT=PASS branch=%s cameras=6 rtsp_sources=6 rtsp_extra=0 detector_workers=1 ui_cameras=CAM-01,CAM-02 ui_hz=%s trt=%s\n' "$BRANCH_EXPECTED" "$V11_UI_PREVIEW_HZ" "$TRT_VERSION"
: >"$LOG"
exec > >(trap '' INT TERM; tee "$LOG") 2>&1
exec "$APP_PY" -u -m services.camera_v11.deepstream_trt86_multi_ui_cam01_cam02_v1
