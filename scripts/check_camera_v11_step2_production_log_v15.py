...
def main() -> int:
    ...
    if not display.is_file() or not detector.is_file():
        print("V11_STEP2_PRODUCTION_V15 FAIL missing_log=1")
        return 2

    display_check = Path(__file__).with_name("check_camera_v11_step1_v7_log.py")
    check = subprocess.run(
        [sys.executable, str(display_check), str(display)],
        text=True,
    ...
    if check.returncode != 0:
        reasons.append("frozen_step1_regression")
    ...
    if reasons:
        print("V11_STEP2_PRODUCTION_V15 RESULT=FAIL reasons=" + ";".join(reasons))
        return 1
    print(
        "V11_STEP2_PRODUCTION_V15 RESULT=PASS "
