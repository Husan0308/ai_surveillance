...
# V3 association/recovery defaults stay frozen.
...
# V4 only affects duplicate-new-ID veto and the published/rendered bbox.
...
fail() { printf 'ML_STEP4_V4_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(f"ML_STEP4_V4_PREFLIGHT ERROR: TensorRT 8.6.1 required, got {trt.__version__}")
if sys.prefix == sys.base_prefix:
    raise SystemExit("ML_STEP4_V4_PREFLIGHT ERROR: TRT86 interpreter is not inside venv")
print(f"ML_STEP4_V4_TRT_ENV python={sys.executable} trt={trt.__version__} numpy={np.__version__}", flush=True)
PY
...
from services.ml_service.app.local_tracker_sparse_v4 import BoxStableObservationRecoveryTracker  # noqa: F401
from services.ml_service.app.detector_substream_tracking_v4 import DetectorSubstreamTrackingV4Service  # noqa: F401
print("ML_STEP4_V4_IMPORTS status=OK detector_v14_frozen=1 v3_association_frozen=1 nested_duplicate_veto=1 render_box_stability=1 bottom_anchor=1 pose=0 gpu_tracker=0", flush=True)
PY

printf '%s\n' \
  "ML_STEP4_V4_PROFILE detector=V14-token3 target=${ML_DETECTOR_TARGET_HZ}Hz/cam rtsp=${ML_SUBSTREAM_RTSP_LATENCY_MS}ms pending_depth=${ML_SUBSTREAM_PENDING_DEPTH}" \
  "ML_STEP4_V4_PROFILE tracker=CPU-v3-association+v4-box-stability nested_ios=${ML_TRACK_NESTED_DUPLICATE_IOS} nested_app=${ML_TRACK_NESTED_DUPLICATE_APP_FLOOR} render_anchor=${ML_TRACK_RENDER_ANCHOR_ALPHA} render_size=${ML_TRACK_RENDER_SIZE_ALPHA}" \
  "ML_STEP4_V4_BOUNDARY camera_service=independent main_stream=0 pose=0 nvdcf=0 gpu_tracker=0 reid=0 global_id=0 api=0 ui=0"

exec "$MAIN_PYTHON" -u -m services.ml_service.app.detector_substream_tracking_v4
