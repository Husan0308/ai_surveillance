...
DISPLAY_LOG="${V11_STEP4_PAIR_DISPLAY_LOG:-/tmp/CAMERA_V11_STEP4_PAIR_DISPLAY.log}"
PAIR_LOG="${V11_STEP4_PAIR_LOG:-/tmp/CAMERA_V11_STEP4_REID_PAIR.log}"
...
fail() {
  printf 'CAMERA_V11_STEP4_REID_PAIR_PREFLIGHT result=FAIL reason=%s\n' "$*" >&2
  exit 1
}
...
CONFLICT_PATTERN='services\.camera_v11\.(step1_cam02_lowlat_v7|step2_production_fp32(_v[0-9]+)?|step3_tracking_v[0-9]+|step4_reid_(quality|gallery|pair)_runtime_v[0-9]+)|yolo26_trt86_step2_worker\.py|reid_trt86_worker_v11\.py'
...
cleanup() {
  (( cleaned == 1 )) && return 0
  cleaned=1
  for pid in "$pair_pid" "$display_pid"; do
...
printf 'CAMERA_V11_STEP4_REID_PAIR_POWER_PRIME phase=baseline start=1\n'
...
sleep 1
printf 'CAMERA_V11_STEP4_REID_PAIR_POWER_PRIME phase=gui_held start=1\n'
"$TRT_PY" "$PRIME_SCRIPT" --engine "$DETECTOR_ENGINE" --warmup 30 --iterations 100 \
...
printf 'CAMERA_V11_STEP4_REID_PAIR_PREFLIGHT result=PASS frozen_sha=%s reid_engine=%s precision=fp32 pair_tsv=%s\n' \
  "d2c9e62f9ed2b5f80dc9a4d496e0fda94afddc51" "$REID_ENGINE" "$PAIR_TSV"

bash "$ROOT/scripts/run_camera_v11_step1_v7.sh" >"$DISPLAY_LOG" 2>&1 &
display_pid=$!
sleep "${V11_STEP4_PAIR_DISPLAY_WARMUP_SEC:-8}"
kill -0 "$display_pid" 2>/dev/null || fail "display_exited_during_warmup"

export V11_STEP2_MODE=full
export V11_STEP2_HZ="${V11_STEP4_PAIR_HZ:-2.0}"
export V11_STEP2_CONF="${V11_STEP4_PAIR_DETECTOR_CONF:-0.18}"
...
"$ROOT/.venv/bin/python" -u -m services.camera_v11.step4_reid_pair_runtime_v1 \
  >"$PAIR_LOG" 2>&1 &
pair_pid=$!

live_ready=0
for _ in $(seq 1 "${V11_STEP4_PAIR_READY_ATTEMPTS:-900}"); do
  if grep -q '^CAMERA_V11_STEP4_REID_PAIR_SCORER_V1 ' "$PAIR_LOG" && \
     grep -q '^CAMERA_V11_STEP4_REID_GALLERY_V1 ' "$PAIR_LOG" && \
     grep -q '^CAMERA_V11_STEP3_V2_TRACKER ' "$PAIR_LOG"; then
    live_ready=1
    break
  fi
...
(( live_ready == 1 )) || fail "pair_runtime_not_ready"

printf 'CAMERA_V11_STEP4_REID_PAIR_RUNNING display_pid=%s pair_pid=%s keeper_pid=%s camera_queue=0 reid_sync=0 scorer_async=1 scorer_gpu=0 max_pairs=24\n' \
  "$display_pid" "$pair_pid" "$V11_POWERMIZER_KEEPER_PID"
while kill -0 "$display_pid" 2>/dev/null && kill -0 "$pair_pid" 2>/dev/null; do
  sleep 1
done
