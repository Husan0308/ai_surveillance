...
POWER_LOG="${V11_STEP4_POWER_LOG:-/tmp/CAMERA_V11_STEP4_POWER_V3.log}"
...
if pgrep -x nvidia-settings >/dev/null 2>&1; then
  fail "nvidia_settings_gui_running_close_it_before_v3"
fi
...
# V3 deliberately does not launch nvidia-settings and does not write
# GPUPowerMizerMode.  On this host/580 driver, keeping nvidia-settings alive can
# coincide with active TensorRT at P0/1721 MHz SM but only 405 MHz memory.  Let
# the driver use its normal floating-clock policy and prove the active workload
# reaches the known-good memory domain before cameras are opened.
printf 'CAMERA_V11_STEP4_REID_MATCH_POWER_V3 mode=natural-floating powermizer_write=0 nvidia_settings_keeper=0 start=1\n'
...
minimum_mhz="${V11_STEP4_MIN_ACTIVE_MEMORY_MHZ:-3000}"
minimum_util="${V11_STEP4_MIN_PRIME_GPU_UTIL:-50}"
printf 'CAMERA_V11_STEP4_REID_MATCH_NATURAL_PRIME peak_memory_mhz=%s peak_sm_mhz=%s peak_gpu_util=%s best_pstate=P%s samples=%s minimum_memory_mhz=%s minimum_gpu_util=%s\n' \
  "$peak_mem" "$peak_sm" "$peak_util" "$best_pstate" "$samples" "$minimum_mhz" "$minimum_util"
if [[ ! "$minimum_mhz" =~ ^[0-9]+$ ]] || [[ ! "$minimum_util" =~ ^[0-9]+$ ]]; then
  fail "invalid_natural_prime_gate"
fi
if (( peak_util < minimum_util )); then
  fail "natural_prime_not_busy_peak_gpu_util_${peak_util}_min_${minimum_util}"
fi
if (( peak_mem < minimum_mhz )); then
  fail "natural_prime_memory_clock_${peak_mem}_min_${minimum_mhz}"
fi
printf 'CAMERA_V11_STEP4_REID_MATCH_NATURAL_PRIME result=PASS active_memory_mhz=%s active_sm_mhz=%s peak_gpu_util=%s\n' \
  "$peak_mem" "$peak_sm" "$peak_util"
printf 'CAMERA_V11_STEP4_REID_MATCH_PREFLIGHT result=PASS frozen_sha=%s reid_engine=%s precision=fp32 pair_tsv=%s match_tsv=%s power_policy=natural-floating active_prime_gate=hard\n' \
...
printf 'CAMERA_V11_STEP4_REID_MATCH_RUNTIME_CLOCK sample=%q diagnostic_only=1\n' "$runtime_sample"
printf 'CAMERA_V11_STEP4_REID_MATCH_RUNNING display_pid=%s match_pid=%s camera_queue=0 reid_sync=0 matcher_async=1 matcher_gpu=0 same_room_only=1 identity_mutation=0 power_policy=natural-floating\n' \
