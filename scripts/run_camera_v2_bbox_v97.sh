...
PY="${CAMERA_V97_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V97_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
# V9.7 keeps V9.6 behavior and changes only display_mux output buffer pool 8 -> 4.
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v97_display_pool import PascalDisplayPoolRuntime  # noqa
print(f"CAMERA_V97_IMPORT python={sys.version.split()[0]} trt={trt.__version__} gi=OK runtime=OK")
PY
then
  fail "Python 3.10 V9.7 runtime import failed"
fi

printf '%s\n' \
  "CAMERA_V97_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V97_PROFILE behavior=v96-unchanged source=${CAMERA_V2_SOURCE_FPS}fps tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V97_POLICY only_change=display-mux-buffer-pool-8-to-4 pts_audit=v95"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v97_display_pool
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V97_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
