...
# Single A/B change: NvDCF visual feature image 18x18 -> 12x12.
export CAMERA_V85_NVDCF_FEATURE_LEVEL="${CAMERA_V85_NVDCF_FEATURE_LEVEL:-1}"

# Exact V8.4 batch-1 detector policy.
...
Gst.init(None)
from services.camera_v2.runtime_v85_nvdcf_relief import PascalNvDCFReliefRuntime  # noqa
PY
  then MAIN_PYTHON="$candidate"; break; fi
done
[[ -n "$MAIN_PYTHON" ]] || fail "no Python can import V8.5 runtime"

export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"
printf '%s\n' \
  "CAMERA_V85_PREFLIGHT status=OK python=$MAIN_PYTHON single_owner=1" \
  "CAMERA_V85_PROFILE source=${CAMERA_V2_SOURCE_FPS}fps tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz nvdcf_feature_level=${CAMERA_V85_NVDCF_FEATURE_LEVEL} detector=batch1-v84" \
  "CAMERA_V85_POLICY one_change=nvdcf-featureImgSizeLevel-2-to-${CAMERA_V85_NVDCF_FEATURE_LEVEL} bbox_unchanged=1 detector_unchanged=1 cadence_unchanged=1 resolution_unchanged=1"

restart_count=0
while true; do
  set +e
  "$MAIN_PYTHON" -u -m services.camera_v2.runtime_v85_nvdcf_relief
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V85_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
