...
PY="${CAMERA_V108_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V108_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
Gst.init(None)
rtspsrc = Gst.ElementFactory.make("rtspsrc", "v108_rtspsrc_probe")
if rtspsrc is None or rtspsrc.find_property("protocols") is None:
    raise SystemExit(12)
import tensorrt as trt
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v108_cam02_udp import PascalCam02UdpRuntime  # noqa
print(
    f"CAMERA_V108_IMPORT python={sys.version.split()[0]} trt={trt.__version__} "
    f"gst={Gst.version_string()} rtspsrc_protocols=1/udp runtime=OK"
)
PY

printf '%s\n' \
  "CAMERA_V108_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V108_PROFILE behavior=v105-plus-cam02-udp tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V108_POLICY only_change=CAM-02-rtp-transport-tcp-to-udp rtsp_latency_ms=${CAMERA_V2_RTSP_LATENCY_MS} udp_buffer_size=${CAMERA_V2_UDP_BUFFER_SIZE:-8388608} batch_size=6 timeout_us=60000 pre_mux_gate=off post_mux_gate=10Hz sync_inputs=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v108_cam02_udp
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V108_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
