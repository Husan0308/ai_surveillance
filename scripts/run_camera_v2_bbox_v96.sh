...
PY="${CAMERA_V96_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V96_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
# V9.6 is one behavior change over V9.5: move the already-latest-only wall queue
# from after tiler to immediately after display_mux. Everything else stays fixed.
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v96_display_freshness import PascalDisplayFreshnessRuntime  # noqa
print(f"CAMERA_V96_IMPORT python={sys.version.split()[0]} trt={trt.__version__} gi=OK runtime=OK")
PY
then
  fail "Python 3.10 V9.6 runtime import failed"
fi

printf '%s\n' \
  "CAMERA_V96_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V96_PROFILE behavior=v95-unchanged source=${CAMERA_V2_SOURCE_FPS}fps tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V96_POLICY only_change=display-latest-queue-moved-after-mux pts_audit=v95"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v96_display_freshness
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V96_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
