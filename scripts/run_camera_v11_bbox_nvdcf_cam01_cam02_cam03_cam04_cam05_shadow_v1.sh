...
LOG="${V11_BBOX_NVDCF_LOG:-/tmp/CAMERA_V11_BBOX_NVDCF_CAM01_CAM02_CAM03_CAM04_CAM05_SHADOW.log}"
APP_PY="${V11_DS_YOLO_PYTHON:-$HOME/ai_surveillance/.venv/bin/python}"
[[ -x "$APP_PY" ]] || APP_PY="$(command -v python3)"
fail(){ printf 'V11_BBOX_NVDCF_CAM01_CAM02_CAM03_CAM04_CAM05_SHADOW_PREFLIGHT RESULT=FAIL reason=%s\n' "$*" >&2; exit 1; }
...
export V11_DS_YOLO_CAMERAS="CAM-01,CAM-02,CAM-03,CAM-04,CAM-05"
export V11_UI_PREVIEW_CAMERAS="CAM-01,CAM-02,CAM-03,CAM-04,CAM-05"
...
# Freeze the accepted detector/NvDCF profile. This step changes only what OSD
# draws after nvtracker. CAM-01..04 keep the six-frame continuity hold; CAM-05
# uses NvDCF's live INACTIVE/shadow tBbox for a bounded short gap instead.
...
# CAM-05 only: draw the tracker-generated shadow bbox for at most 10 frames
# (~0.5 s at 20 FPS) after an already-active ID becomes INACTIVE. Never draw a
# shadow target that was not active first; suppress it if a current box overlaps.
export V11_BBOX_SHADOW_DIAG_CAMERAS="${V11_BBOX_SHADOW_DIAG_CAMERAS:-CAM-05}"
export V11_BBOX_SHADOW_DISPLAY_CAMERAS="${V11_BBOX_SHADOW_DISPLAY_CAMERAS:-CAM-05}"
...
printf 'V11_BBOX_NVDCF_CAM01_CAM02_CAM03_CAM04_CAM05_SHADOW_PREFLIGHT RESULT=PASS branch=%s cameras=CAM-01,CAM-02,CAM-03,CAM-04,CAM-05 rtsp_sources=5 detector_workers=1 detector_hz=%s raw_conf=%s tracker=nvdcf tracker_size=%sx%s reid=0 display_track_conf=%s cam01_04_hold_frames=%s cam05_shadow_frames=%s cam05_shadow_min_conf=%s cam05_shadow_iou=%s config=%s trt=%s log=%s\n' "$BRANCH_EXPECTED" "$V11_DS_YOLO_HZ" "$V11_DS_YOLO_CONF" "$V11_BBOX_TRACKER_WIDTH" "$V11_BBOX_TRACKER_HEIGHT" "$CAMERA_V2_MIN_DISPLAY_TRACK_CONF" "$V11_BBOX_DISPLAY_HOLD_FRAMES" "$V11_BBOX_SHADOW_DISPLAY_FRAMES" "$V11_BBOX_SHADOW_DISPLAY_MIN_CONF" "$V11_BBOX_SHADOW_DISPLAY_SUPPRESS_IOU" "$TRACKER_CFG" "$TRT_VERSION" "$LOG"
: >"$LOG"
