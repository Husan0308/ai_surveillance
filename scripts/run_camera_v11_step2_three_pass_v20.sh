...
OUT="${V11_STEP2_THREE_PASS_OUT:-/tmp/camera_v11_step2_three_pass_v20}"
DURATION="${V11_STEP2_THREE_PASS_DURATION_SEC:-60}"
WARMUP="${V11_STEP2_THREE_PASS_DISPLAY_WARMUP_SEC:-8}"
mkdir -p "$OUT"
...
for run_number in 1 2 3; do
  ...
  printf 'CAMERA_V11_STEP2_THREE_PASS_START run=%s duration=%ss display_warmup=%ss analytics_nice=%s\n' \
  ...
  "$ROOT/scripts/run_camera_v11_step2_stage_v20.sh" full >"$detector_log" 2>&1 &
  detector_pid=$!
  ready=0
  ...
  dmon_pid=$!
  pidstat -h -u -r -p "$display_pid,$detector_pid,$worker_pid" 1 >"$pidstat_log" 2>&1 &
  pidstat_pid=$!
  sleep "$DURATION"
  cleanup
  ...
  if (( check_status != 0 )); then
    printf 'CAMERA_V11_STEP2_THREE_PASS_RESULT run=%s result=FAIL\n' "$run_number" | tee -a "$check_log"
    failed=1
    break
  fi
  printf 'CAMERA_V11_STEP2_THREE_PASS_RESULT run=%s result=PASS\n' "$run_number" | tee -a "$check_log"
done

if (( failed == 0 )); then
  printf 'CAMERA_V11_STEP2_THREE_PASS_FINAL result=PASS consecutive=3 duration=%ss analytics_nice=%s\n' \
    "$DURATION" "${V11_STEP2_NICE:-10}"
else
  printf 'CAMERA_V11_STEP2_THREE_PASS_FINAL result=FAIL duration=%ss analytics_nice=%s\n' \
    "$DURATION" "${V11_STEP2_NICE:-10}"
fi
exit "$failed"
