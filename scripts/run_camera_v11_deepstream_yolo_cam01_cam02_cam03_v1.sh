...
cd "$(dirname "$0")/.."
ROOT="$PWD"
BRANCH_EXPECTED="rebuild/service-architecture-v11-deepstream-yolo-cam01-cam02-cam03-v1-20260901"
LOG="${V11_DS_YOLO_LOG:-/tmp/CAMERA_V11_DS_YOLO_CAM01_CAM02_CAM03.log}"
...
CAMERAS="${V11_DS_YOLO_CAMERAS:-CAM-01,CAM-02,CAM-03}"
...
[[ "$CAMERAS" == "CAM-01,CAM-02,CAM-03" ]] || fail "camera_set=$CAMERAS expected=CAM-01,CAM-02,CAM-03"
...
printf 'V11_DS_YOLO_MULTI_GPU_POLICY cameras=3 rtsp_sources=3 rtsp_per_camera=1 decode=deepstream-nvdec detector=shared-trt86-sidecar detector_workers=1 detector_rtsp=0 detector_queue=latest1-per-camera scheduler=round-robin gst_nvinfer=0 trt10=0 second_rtsp=0 opencv=0 ffmpeg=0 model_pt_required=0 log=%s\n' "$LOG"
