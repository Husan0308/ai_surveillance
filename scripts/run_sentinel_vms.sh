...
echo "SENTINEL_BUILD branch=${BRANCH} head=${HEAD_SHA} expected_ui=2026.08.19-r13"
...
# Build the wall in the final production topology before PLAYING. The UI also
# sends one focus=-1 grid-prime command after process startup to force the same
# renegotiation that previously happened only after a manual fullscreen action.
