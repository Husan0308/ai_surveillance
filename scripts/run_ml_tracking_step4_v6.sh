...
export ML_TRACK_LOW_RECOVERY_SEC="${ML_TRACK_LOW_RECOVERY_SEC:-1.8}"
...
export ML_TRACK_LOST_LOW_JUMP_DIAG="${ML_TRACK_LOST_LOW_JUMP_DIAG:-1.05}"
export ML_TRACK_LOST_HIGH_JUMP_DIAG="${ML_TRACK_LOST_HIGH_JUMP_DIAG:-1.35}"

export ML_TRACK_RENDER_EXPAND_ALPHA="${ML_TRACK_RENDER_EXPAND_ALPHA:-0.82}"
export ML_TRACK_RENDER_CONTRACT_ALPHA="${ML_TRACK_RENDER_CONTRACT_ALPHA:-0.14}"
export ML_TRACK_RENDER_EXPAND_MAX_STEP="${ML_TRACK_RENDER_EXPAND_MAX_STEP:-0.72}"
export ML_TRACK_RENDER_CONTRACT_MAX_STEP="${ML_TRACK_RENDER_CONTRACT_MAX_STEP:-0.22}"
export ML_TRACK_ENVELOPE_PAD_X="${ML_TRACK_ENVELOPE_PAD_X:-0.07}"
export ML_TRACK_ENVELOPE_PAD_TOP="${ML_TRACK_ENVELOPE_PAD_TOP:-0.04}"
export ML_TRACK_ENVELOPE_PAD_BOTTOM="${ML_TRACK_ENVELOPE_PAD_BOTTOM:-0.03}"
export ML_TRACK_ENVELOPE_COMPACT_EXTRA_X="${ML_TRACK_ENVELOPE_COMPACT_EXTRA_X:-0.06}"
...
fail() { printf 'ML_STEP4_V6_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(f"ML_STEP4_V6_PREFLIGHT ERROR: TensorRT 8.6.1 required, got {trt.__version__}")
if sys.prefix == sys.base_prefix:
    raise SystemExit("ML_STEP4_V6_PREFLIGHT ERROR: TRT86 interpreter is not inside venv")
print(f"ML_STEP4_V6_TRT_ENV python={sys.executable} trt={trt.__version__} numpy={np.__version__}", flush=True)
PY
...
from services.ml_service.app.local_tracker_sparse_v6 import BodyEnvelopeObservationRecoveryTracker  # noqa: F401
from services.ml_service.app.detector_substream_tracking_v6 import DetectorSubstreamTrackingV6Service  # noqa: F401
print("ML_STEP4_V6_IMPORTS status=OK low_score=1 no_teleport=1 body_envelope=1 raw_inside_render=1 latency_stamp=1 pose=0 gpu_tracker=0", flush=True)
PY

printf '%s\n' \
  "ML_STEP4_V6_PROFILE detector_conf=${ML_DETECTOR_CONF} low=${ML_TRACK_LOW_THRESH} high=${ML_TRACK_HIGH_THRESH} new=${ML_TRACK_NEW_THRESH} target=${ML_DETECTOR_TARGET_HZ}Hz/cam" \
  "ML_STEP4_V6_PROFILE low_recovery=${ML_TRACK_LOW_RECOVERY_SEC}s expand=${ML_TRACK_RENDER_EXPAND_ALPHA} contract=${ML_TRACK_RENDER_CONTRACT_ALPHA} pad_x=${ML_TRACK_ENVELOPE_PAD_X}+compact" \
  "ML_STEP4_V6_BOUNDARY camera_service=independent detector=substream tracker=cpu-local render=body-envelope pose=0 nvdcf=0 reid=0 global_id=0"

exec "$MAIN_PYTHON" -u -m services.ml_service.app.detector_substream_tracking_v6
