...
# Keep the proven six-camera Pascal-safe display/RTSP path unchanged.
...
# Preserve CCTV aspect before Ultralytics; YOLO owns final letterbox/preprocess.
...
# Detection is the only subsystem under test. Increase cadence enough that every
# camera receives frequent person truth while still leaving headroom for display.
export CAMERA_V2_DETECT_GPU_DUTY=0.46
export CAMERA_V2_DETECT_GPU_DUTY_MIN=0.38
export CAMERA_V2_DETECT_GPU_DUTY_MAX=0.58

# Latest raw detector result is shown directly; this is not motion prediction.
# The hold only bridges the round-robin inference interval between six cameras.
export CAMERA_V2_RAW_BOX_MAX_AGE=2.40
...
# Detailed truth telemetry remains enabled until detection is proven visually.
export CAMERA_V2_STABLE_YOLO_LOG_BUDGET=120

echo "SENTINEL_PROFILE detector=YOLO26m person_only=classes[0] mode=one-to-many-nms capture=704x400 model_imgsz=704x448 conf=0.08 iou=0.50 batch=2 tracker=OFF flow=OFF raw_meta=ON camera_path=pascal-safe-analysis-tiler display=egl"
