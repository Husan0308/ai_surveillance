#!/usr/bin/env python3
"""Fail closed on weak CAM-01 + CAM-02 + CAM-03 + CAM-04 + CAM-05 DeepStream 9.1 soak evidence."""
import argparse
import csv
import json
...
def assess(source_rows, group_rows, text, gpu):
    failures = []
    if set(source_rows) != {"CAM-01", "CAM-02", "CAM-03", "CAM-04", "CAM-05"}:
    ...
    if len(stable_group) < 2:
        ...
        # With five asynchronous live sources and sync-inputs=false, downstream
    ...
    if "nvstreammux(batch=5" not in text or "inference=0" not in text:
        failures.append("Missing five-source mux/no-inference evidence")
    for cid in ("CAM-01", "CAM-02", "CAM-03", "CAM-04", "CAM-05"):
...
def main() -> int:
    ...
    probe = subprocess.run(
        [
            ...
            "-of",
            "json",
            str(args.directory / "CAM-01_CAM-02_CAM-03_CAM-04_CAM-05.mkv"),
        ],
        capture_output=True,
        text=True,
        timeout=60,
