...
cd "$(dirname "$0")/.."
ROOT="$PWD"
OUT="${V11_STEP4_MATCH_ACCEPTANCE_OUT:-/tmp/camera_v11_step4_reid_same_room_matcher_v3}"
...
V11_STEP4_MATCH_RUN_SEC="$DURATION" \
  ...
  bash "$ROOT/scripts/run_camera_v11_step4_reid_same_room_matcher_v3.sh" \
  >"$LAUNCHER_LOG" 2>&1
status=$?
if (( status != 0 && status != 124 && status != 130 && status != 143 )); then
  tail -n 140 "$LAUNCHER_LOG" >&2 || true
  tail -n 180 "$MATCH_LOG" >&2 || true
  fail "launcher_status_$status"
fi

grep -q '^CAMERA_V11_STEP4_REID_MATCH_NATURAL_PRIME result=PASS ' "$LAUNCHER_LOG" \
  || fail "natural_prime_not_passed"
grep -q '^CAMERA_V11_STEP4_REID_MATCH_PREFLIGHT result=PASS ' "$LAUNCHER_LOG" \
...
if grep -q 'nvidia_settings_keeper=1\|powermizer_write=1' "$LAUNCHER_LOG"; then
  fail "powermizer_mutation_forbidden_in_v3"
fi
