...
LOCK_FILE="/tmp/ai_surveillance_camera_v11_step2_production_v21.lock"
...
fail() {
  printf 'CAMERA_V11_STEP2_V21_PREFLIGHT ERROR: %s\n' "$*" >&2
  exit 1
}

command -v flock >/dev/null 2>&1 || fail "flock missing"
exec 8>"$LOCK_FILE"
flock -n 8 || fail "another Step2 production launcher holds $LOCK_FILE"
[[ -n "${DISPLAY:-}" ]] || fail "DISPLAY is empty"
[[ -s "$ROOT/artifacts/yolo26s_trt86/yolo26s-672x384-b1-fp32-trt86.engine" ]] || \
  fail "FP32 engine missing"
...
"$ROOT/scripts/prime_camera_v11_trt86_memory_clock_v20.sh" || \
  fail "TRT memory-clock preflight failed"
...
: >"$DISPLAY_LOG"
: >"$DETECTOR_LOG"
printf 'CAMERA_V11_STEP2_V21_PREFLIGHT status=OK base=%s precision=fp32 analytics_nice=%s display_log=%s detector_log=%s\n' \
...
detector_pid=$!
printf 'CAMERA_V11_STEP2_V21_RUNNING display_pid=%s detector_pid=%s\n' "$display_pid" "$detector_pid"
