  ...
  for (auto *node = meta->frame_meta_list; node; node = node->next) {
    ...
    for (auto *obj_node = frame->obj_meta_list; obj_node; obj_node = obj_node->next) {
      ...
      if (detection_evidence)
        std::fprintf(detection_evidence,
          "{\"source_id\":%u,\"frame\":%d,\"pts_ns\":%lu,\"class_id\":%d,\"confidence\":%.6f,\"box\":[%.3f,%.3f,%.3f,%.3f]}\n",
          frame->source_id, frame->frame_num, frame->buf_pts, obj->class_id,
          obj->confidence, r.left, r.top, r.width, r.height);
    }
    if (frame_evidence)
      std::fprintf(frame_evidence,"{\"source_id\":%u,\"frame\":%d,\"pts_ns\":%lu,\"batch_pts_ns\":%lu,\"persons\":%u}\n",
        frame->source_id,frame->frame_num,frame->buf_pts,GST_BUFFER_PTS(b),count);
    c.persons += count;
