// CAM-01 + CAM-02 + CAM-03 + CAM-04 DeepStream 9.1 validation. No inference, tracker, OpenCV or CPU pixel copies.
#include <gst/gst.h>
#include <glib-unix.h>
#include <atomic>
#include <algorithm>
...
static gboolean tick(gpointer) {
  ...
  if (interruption_resumed && !interruption_reported) {
    ...
    if (target && target->input.frames.load() >= target_resume_frames + 100) {
      ...
      const bool ok = peer_off_gain >= guint64(interrupt_seconds * 45) && target_off_gain <= 5;
      g_print(
        "GROUP ISOLATION_SUMMARY target=%s healthy_peers=3 outage_sec=%d peer_frames_during=%lu "
...
int main(int argc, char **argv) {
  if (argc != 3) {
    g_printerr("usage: cam-four-validator CONFIG DURATION\n");
    return 2;
  }
  ...
  if (!cam01 || !cam02 || !cam03 || !cam04) {
    g_printerr("GROUP camera config invalid\n");
    return 2;
  }
  sources = {cam01, cam02, cam03, cam04};
  ...
  pipeline = gst_pipeline_new("CAM-01-CAM-02-CAM-03-CAM-04-validation");
  ...
  g_object_set(tiler, "rows", 2u, "columns", 2u, "width", 1920u, "height", 1080u, nullptr);
  GstElement *conv = make("nvvideoconvert", "converter");
  GstElement *capsfilter = make("capsfilter", "nvmm-caps");
  GstCaps *caps = gst_caps_from_string("video/x-raw(memory:NVMM),format=NV12,width=1920,height=1080");
  g_object_set(capsfilter, "caps", caps, nullptr);
  gst_caps_unref(caps);
  encoder = make("nvv4l2h264enc", "NVENC");
  g_object_set(encoder, "bitrate", 16000000u, "iframeinterval", 40u, nullptr);
  ...
  g_object_set(sink_element, "location", "/work/CAM-01_CAM-02_CAM-03_CAM-04.mkv", "sync", FALSE,
  ...
  event(nullptr, "GRAPH",
        std::string("CAM-01+CAM-02+CAM-03+CAM-04 nvurisrcbin/NVDEC->NVMM->queues->nvstreammux(batch=4,2560x1440,live,50000us,sync-inputs=false)->tiler(2x2,1920x1080)->NVENC->record") +
