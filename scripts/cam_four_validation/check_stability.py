#!/usr/bin/env python3
"""Fail closed on weak CAM-01 + CAM-02 + CAM-03 + CAM-04 DeepStream 9.1 soak evidence."""
import argparse
import csv
import json
...
def assess(source_rows, group_rows, text, gpu):
    failures = []
    if set(source_rows) != {"CAM-01", "CAM-02", "CAM-03", "CAM-04"}:
    ...
    if len(stable_group) < 2:
        ...
        # With four asynchronous live sources and sync-inputs=false, downstream
    ...
    if "nvstreammux(batch=4" not in text or "inference=0" not in text:
        failures.append("Missing four-source mux/no-inference evidence")
...
def main() -> int:
    ...
    probe = subprocess.run(
        [
            ...
            "-of",
            "json",
            str(args.directory / "CAM-01_CAM-02_CAM-03_CAM-04.mkv"),
        ],
        capture_output=True,
        text=True,
        timeout=60,
    ...
    try:
        ...
        if stream["width"] != 1920 or stream["height"] != 1080 or stream["codec_name"] != "h264":
