...
PY="${CAMERA_V105_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V105_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v105_mux_arrival_audit import PascalTrackerMuxArrivalAuditRuntime  # noqa
print(f"CAMERA_V105_IMPORT python={sys.version.split()[0]} trt={trt.__version__} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V105_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V105_PROFILE behavior=v104-unchanged audit=per-source-mux-arrival tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V105_POLICY behavior_change=0 batch_size=6 timeout_us=60000 pre_mux_gate=off post_mux_gate=10Hz sync_inputs=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v105_mux_arrival_audit
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V105_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
