...
# Step 4 v2: sparse-detector local tracking. CPU-only, per-camera.
...
export ML_TRACK_LOW_RECOVERY_SEC="${ML_TRACK_LOW_RECOVERY_SEC:-1.6}"
...
fail() { printf 'ML_STEP4_V2_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(f"ML_STEP4_V2_PREFLIGHT ERROR: TensorRT 8.6.1 required, got {trt.__version__}")
if sys.prefix == sys.base_prefix:
    raise SystemExit(
        "ML_STEP4_V2_PREFLIGHT ERROR: TRT86 interpreter is not inside venv: "
...
from services.ml_service.app.local_tracker_sparse_v2 import SparseRecoveryPersonTracker  # noqa: F401
from services.ml_service.app.detector_substream_tracking_v2 import DetectorSubstreamTrackingV2Service  # noqa: F401
print(
    "ML_STEP4_V2_IMPORTS status=OK detector_v14_frozen=1 cpu_tracker=1 "
    "sparse_recovery=1 low_lost_recovery=1 duplicate_suppression=1 gpu_tracker=0",
    flush=True,
)
PY

printf '%s\n' \
  "ML_STEP4_V2_PROFILE detector=V14-token3 target=${ML_DETECTOR_TARGET_HZ}Hz/cam rtsp=${ML_SUBSTREAM_RTSP_LATENCY_MS}ms pending_depth=${ML_SUBSTREAM_PENDING_DEPTH}" \
  "ML_STEP4_V2_PROFILE tracker=CPU-sparse-recovery low=${ML_TRACK_LOW_THRESH} high=${ML_TRACK_HIGH_THRESH} new=${ML_TRACK_NEW_THRESH} reacquire=${ML_TRACK_REACQUIRE_THRESH} low_recovery=${ML_TRACK_LOW_RECOVERY_THRESH}/${ML_TRACK_LOW_RECOVERY_SEC}s shadow=${ML_TRACK_SHADOW_SEC}s" \
  "ML_STEP4_V2_BOUNDARY camera_service=independent main_stream=0 nvdcf=0 gpu_tracker=0 reid=0 global_id=0 api=0 ui=0"

exec "$MAIN_PYTHON" -u -m services.ml_service.app.detector_substream_tracking_v2
