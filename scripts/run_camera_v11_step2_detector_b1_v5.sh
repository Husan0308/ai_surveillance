...
LOCK_FILE="/tmp/ai_surveillance_camera_v11_step2_b1_v5.lock"
...
fail() { printf 'CAMERA_V11_STEP2V5_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
command -v flock >/dev/null 2>&1 || fail "flock missing"
exec 9>"$LOCK_FILE"
flock -n 9 || fail "another V11 Step2 V5 runtime holds $LOCK_FILE; stop it first"
...
# Step2 V5: latency-first batch1, one camera at a time.
...
export V11_DETECT_B1_HZ_PER_CAMERA="${V11_DETECT_B1_HZ_PER_CAMERA:-3.0}"
export V11_DETECT_B1_CAPTURE_TIMEOUT_MS="${V11_DETECT_B1_CAPTURE_TIMEOUT_MS:-140}"

"$PY" - <<'PY'
import sys
if sys.version_info[:2] != (3, 10):
    raise SystemExit(f"V11 Step2 V5 requires Python 3.10, got {sys.version}")
import gi
gi.require_version("Gst", "1.0")
from gi.repository import Gst
Gst.init(None)
from services.camera_v11.batch1_trt86 import Batch1TRT86Client  # noqa: F401
from services.camera_v11.step2_detector_b1_v5 import V11Step2DetectorB1V5  # noqa: F401
print(f"CAMERA_V11_STEP2V5_IMPORT python={sys.version.split()[0]} gst={Gst.version_string()} runtime=OK")
PY
...
printf '%s\n' \
  ...
  "CAMERA_V11_STEP2V5_PREFLIGHT status=OK python=$PY display=${DISPLAY} engine=$ENGINE worker=$WORKER" \
  "CAMERA_V11_STEP2V5_INVARIANT base=step1-v7-frozen batch=1 round_robin=1 detector_queue=1 latest_only=1 tracker=0 osd=0 reid=0 face=0" \
  "CAMERA_V11_STEP2V5_TARGET per_camera=${V11_DETECT_B1_HZ_PER_CAMERA}Hz trt_p95_target=45ms result_age_target=140ms"

exec "$PY" -u -m services.camera_v11.step2_detector_b1_v5
