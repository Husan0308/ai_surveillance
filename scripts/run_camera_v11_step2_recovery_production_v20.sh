...
LOCK_FILE="/tmp/ai_surveillance_camera_v11_step2_recovery_v20.lock"
DISPLAY_LOG="${V11_STEP2_DISPLAY_LOG:-/tmp/CAMERA_V11_STEP2_RECOVERY_DISPLAY.log}"
DETECTOR_LOG="${V11_STEP2_DETECTOR_LOG:-/tmp/CAMERA_V11_STEP2_RECOVERY_DETECTOR.log}"

fail() {
  printf 'CAMERA_V11_STEP2_RECOVERY_PREFLIGHT result=FAIL reason=%s\n' "$*" >&2
  exit 1
}
...
[[ -s "$ROOT/artifacts/yolo26s_trt86/yolo26s-672x384-b1-fp32-trt86.engine" ]] || fail "fp32_engine_missing"
git diff --quiet "$BASE" -- services/camera_v11/step1_cam02_lowlat_v7.py \
  ...
  scripts/check_camera_v11_step1_v7_log.py || fail "frozen_step1_differs"
"$ROOT/scripts/prime_camera_v11_trt86_memory_clock_v20.sh" || fail "memory_clock_preflight"
...
: >"$DISPLAY_LOG"
: >"$DETECTOR_LOG"
printf 'CAMERA_V11_STEP2_RECOVERY_PREFLIGHT result=PASS base=%s precision=fp32 batch=1 analytics_nice=%s display_policy=bounded-backpressure-depth1 display_log=%s detector_log=%s\n' \
  "$BASE" "${V11_STEP2_NICE:-10}" "$DISPLAY_LOG" "$DETECTOR_LOG"
...
detector_pid=$!
printf 'CAMERA_V11_STEP2_RECOVERY_RUNNING display_pid=%s detector_pid=%s\n' "$display_pid" "$detector_pid"
