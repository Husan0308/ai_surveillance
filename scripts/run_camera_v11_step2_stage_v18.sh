...
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$PWD"; MODE="${1:-full}"
case "$MODE" in
  extraction|preprocessing|full) ;;
  synthetic-trt) exec "$ROOT/scripts/run_camera_v11_step2_stage.sh" synthetic-trt ;;
...
exec "$ROOT/.venv/bin/python" -u -m services.camera_v11.step2_production_fp32_v18
