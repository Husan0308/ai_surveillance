...
LOG="${V11_BBOX_NVDCF_LOG:-/tmp/CAMERA_V11_BBOX_NVDCF_CAM01_CAM02_CAM03_CAM04.log}"
APP_PY="${V11_DS_YOLO_PYTHON:-$HOME/ai_surveillance/.venv/bin/python}"
[[ -x "$APP_PY" ]] || APP_PY="$(command -v python3)"
fail(){ printf 'V11_BBOX_NVDCF_CAM01_CAM02_CAM03_CAM04_PREFLIGHT RESULT=FAIL reason=%s\n' "$*" >&2; exit 1; }
...
export V11_DS_YOLO_CAMERAS="CAM-01,CAM-02,CAM-03,CAM-04"
export V11_UI_PREVIEW_CAMERAS="CAM-01,CAM-02,CAM-03,CAM-04"
...
export V11_BBOX_TRACK_CAMERAS="CAM-01,CAM-02,CAM-03,CAM-04"
# CAM-04 rollout is intentionally additive: keep the accepted CAM-01/02/03
# detector, tracker, display-confidence and latency profile unchanged.
...
"$APP_PY" -c 'from services.ml_service.app.config import load_settings; rows=load_settings().cameras; wanted={"CAM-01","CAM-02","CAM-03","CAM-04"}; found={c.camera_id for c in rows if c.username and c.password}; assert wanted <= found' || fail "camera_credentials_unresolved"
printf 'V11_BBOX_NVDCF_CAM01_CAM02_CAM03_CAM04_PREFLIGHT RESULT=PASS branch=%s cameras=CAM-01,CAM-02,CAM-03,CAM-04 rtsp_sources=4 detector_workers=1 detector_hz=%s raw_conf=%s tracker=nvdcf tracker_size=%sx%s reid=0 display_margin=0/0/0 display_track_conf=%s profile=responsive-dynamic config=%s trt=%s log=%s\n' "$BRANCH_EXPECTED" "$V11_DS_YOLO_HZ" "$V11_DS_YOLO_CONF" "$V11_BBOX_TRACKER_WIDTH" "$V11_BBOX_TRACKER_HEIGHT" "$CAMERA_V2_MIN_DISPLAY_TRACK_CONF" "$TRACKER_CFG" "$TRT_VERSION" "$LOG"
: >"$LOG"
