...
cd "$(dirname "$0")/.."
ROOT="$PWD"
OUT="${V11_STEP5_ACCEPTANCE_OUT:-/tmp/camera_v11_step5_global_shadow_v1}"
DURATION="${V11_STEP5_DURATION_SEC:-60}"
...
fail() {
  printf 'V11_STEP5_GLOBAL_SHADOW_V1 RESULT=FAIL reason=%s production_global_id=0 room_id=0 face=0 handoff=0 identity_accuracy_proven=0\n' "$*" >&2
  exit 1
}
...
"$ROOT/.venv/bin/python" "$ROOT/scripts/test_camera_v11_step5_global_shadow_v1.py" \
  2>&1 | tee "$OUT/step5_unit.log"
(( PIPESTATUS[0] == 0 )) || fail "step5_unit_tests"
"$ROOT/.venv/bin/python" "$ROOT/scripts/check_camera_v11_frozen_step123_guard.py" \
  2>&1 | tee "$OUT/frozen_guard.log"
...
: >"$LAUNCHER_LOG"
: >"$CHECK_LOG"
V11_STEP5_DISPLAY_LOG="$DISPLAY_LOG" \
V11_STEP5_GLOBAL_LOG="$MATCH_LOG" \
V11_STEP4_PAIR_TSV="$PAIR_TSV" \
V11_STEP4_MATCH_TSV="$MATCH_TSV" \
V11_STEP5_GLOBAL_TSV="$GLOBAL_TSV" \
V11_STEP5_RUN_SEC="$DURATION" \
  timeout -s TERM "$((DURATION + 75))s" \
  bash "$ROOT/scripts/run_camera_v11_step5_global_shadow_v1.sh" \
  >"$LAUNCHER_LOG" 2>&1
status=$?
if (( status != 0 && status != 124 && status != 130 && status != 143 )); then
  tail -n 140 "$LAUNCHER_LOG" >&2 || true
  tail -n 220 "$MATCH_LOG" >&2 || true
  fail "launcher_status_$status"
fi

grep -q '^CAMERA_V11_STEP5_GLOBAL_SHADOW_NATURAL_PRIME result=PASS ' "$LAUNCHER_LOG" \
  || fail "natural_prime_not_passed"
grep -q '^CAMERA_V11_STEP5_GLOBAL_SHADOW_PREFLIGHT result=PASS ' "$LAUNCHER_LOG" \
  || fail "no_preflight_pass"
grep -q 'power_policy=natural-floating' "$LAUNCHER_LOG" \
  || fail "natural_floating_policy_missing"
if grep -q 'nvidia_settings_keeper=1\|powermizer_write=1' "$LAUNCHER_LOG"; then
  fail "powermizer_mutation_forbidden"
fi

"$ROOT/.venv/bin/python" "$ROOT/scripts/check_camera_v11_step5_global_shadow_v1_log.py" \
