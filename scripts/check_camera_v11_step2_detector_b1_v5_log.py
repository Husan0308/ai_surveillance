...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: check_camera_v11_step2_detector_b1_v5_log.py /tmp/CAMERA_V11_STEP2V5.log")
        return 2
    log_path = Path(sys.argv[1])
    if not log_path.is_file():
        print(f"V11_STEP2V5 FAIL missing_log={log_path}")
        return 2
    text = log_path.read_text(encoding="utf-8", errors="replace")

    for marker in (
        "CAMERA_V11_STEP2V5_PREFLIGHT",
        "CAMERA_V11_STEP2V5_INVARIANT",
        "CAMERA_V11_STEP2V5_TARGET",
        "CAMERA_V11_STEP2V5_ARCH",
        "CAMERA_V11_STEP2V5_POLICY",
        "CAMERA_V11_STEP2V5_DETECT_READY",
    ):
        if marker not in text:
            print(f"V11_STEP2V5 FAIL missing={marker}")
            return 2

    # Frozen display is a hard gate. Detector tuning is irrelevant until this passes.
    ...
    if proc.returncode != 0:
        print("V11_STEP2V5 RESULT diagnosis=FAIL_STEP1_REGRESSION")
        print("V11_STEP2V5 next=do not add tracker; reduce only detector duty or conversion cost")
        return 1
    ...
    if len(latest_cam) != 6 or not rows:
        print(f"V11_STEP2V5 FAIL detector_camera_rows={len(latest_cam)} v5_stats_rows={len(rows)}")
        return 2
    ...
    for cid, m in sorted(latest_cam.items()):
        ...
        print(
            "V11_STEP2V5_CAMERA "
            f"camera={cid} rate={actual_rate:.2f}Hz base_reported={reported_hz:.2f}Hz "
            f"captures={captures} results={results} conversion_p95={conversion95:.1f}ms "
            f"result_age_p95={camera_age95:.1f}ms qmax={qmax}"
        )

    print(
        "V11_STEP2V5_DETECTOR "
    ...
    if reasons:
        print("V11_STEP2V5 RESULT diagnosis=FAIL_B1_DETECTOR reasons=" + ";".join(reasons))
        print("V11_STEP2V5 next=do not add tracker; tune only batch1 detector duty/capture path")
        return 1

    print("V11_STEP2V5 RESULT diagnosis=PASS_B1_DETECTOR_ONLY")
    print("V11_STEP2V5 next=freeze Step2 V5; then add tracker in Step3 without changing display/detector")
    return 0
