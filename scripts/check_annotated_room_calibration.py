...
from services.camera_v2.annotated_geometry_reid import AnnotatedRoomCalibration
...
def main() -> int:
    calibration = AnnotatedRoomCalibration()
    ...
    for source in snap["camera_sources"]:
        ...
        print(
            f"source={source} room={row.room_id} model={row.model} "
            f"reprojection_rmse={row.reprojection_rmse_m:.5f} units={row.units}"
        )
