#!/usr/bin/env python3
"""Validate CAM-01 + CAM-02 + CAM-03 together on DeepStream 9.1 with no AI."""
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / ".runtime/cam01-cam02-cam03-validation")
    ap.add_argument("--interrupt-camera", choices=["none", "CAM-01", "CAM-02", "CAM-03"], default="none")
    ...
    wanted = {"CAM-01", "CAM-02", "CAM-03"}
    cameras = {c.camera_id: c for c in settings.cameras if c.camera_id in wanted}
    if set(cameras) != wanted:
        raise RuntimeError("CAM-01, CAM-02 and CAM-03 must all exist and be enabled")
    ...
    build = base + [
        "--network=none",
        "-v", f"{ROOT / 'scripts/cam_three_validation'}:/src:ro",
        "-v", f"{out}:/out",
        "--entrypoint", "bash",
        image,
        "-c",
        "g++ -O2 -std=c++17 -Wall -Wextra /src/main.cpp -o /out/cam-three-validator "
        "$(pkg-config --cflags --libs gstreamer-1.0)",
    ]
    subprocess.run(build, check=True)

    secret = out / "camera-three-secret.ini"
    ...
    container = "ai-surveillance-cam-three-validation"
    cmd = base + [
        "--name", container,
        ...
        "-e", "GST_REGISTRY=/tmp/cam-three-gst-registry.bin",
        ...
        *([] if not preview_enabled else ["--add-host", "host.docker.internal:host-gateway"]),
        "-v", f"{out}:/work",
        "--entrypoint", "/work/cam-three-validator",
        image,
        "/work/camera-three-secret.ini",
        str(args.duration),
    ]
        ...
        if preview_enabled:
            ...
            preview = subprocess.Popen(
                [
                    ...
                    "-an",
                    "-sync", "video",
                    "-window_title", "CAM-01 + CAM-02 + CAM-03 LIVE",
                    preview_url,
                ],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
