...
cd "$(dirname "$0")/.."
ROOT="$PWD"
BRANCH_EXPECTED="rebuild/service-architecture-v11-deepstream-yolo-cam01-cam02-v1-20260901"
CAMERAS="${V11_DS_YOLO_CAMERAS:-CAM-01,CAM-02}"
LOG="${V11_DS_YOLO_LOG:-/tmp/CAMERA_V11_DS_YOLO_CAM01_CAM02.log}"
...
"$APP_PY" -c 'import services.camera_v11.deepstream_trt86_multi_v1' \
  || fail "runtime_import_failed"
"$APP_PY" - <<'PY' || fail "camera_credentials_unresolved"
import os
from services.ml_service.app.config import load_settings
wanted = [x.strip() for x in os.environ["V11_DS_YOLO_CAMERAS"].split(",") if x.strip()]
rows = {c.camera_id: c for c in load_settings().cameras}
assert wanted
assert all(cid in rows for cid in wanted)
assert all(rows[cid].username and rows[cid].password for cid in wanted)
PY
...
EXPECTED_RTSP="${#camera_array[@]}"
[[ "$EXPECTED_RTSP" -ge 1 ]] || fail "zero_cameras"

printf 'V11_DS_YOLO_MULTI_PREFLIGHT RESULT=PASS branch=%s cameras=%s rtsp_sources=%s detector_workers=1 architecture=deepstream-per-camera-single-rtsp+shared-trt86 trt=%s engine=%s hz=%s conf=%s\n' \
  "$BRANCH_EXPECTED" "$CAMERAS" "$EXPECTED_RTSP" "$TRT_VERSION" "$ENGINE" "$V11_DS_YOLO_HZ" "$V11_DS_YOLO_CONF"
printf 'V11_DS_YOLO_MULTI_GPU_POLICY cameras=%s rtsp_sources=%s rtsp_per_camera=1 decode=deepstream-nvdec detector=shared-trt86-sidecar detector_workers=1 detector_rtsp=0 detector_queue=latest1-per-camera gst_nvinfer=0 trt10=0 trt86=%s second_rtsp=0 opencv=0 ffmpeg=0 model_pt_required=0 log=%s\n' \
  "$CAMERAS" "$EXPECTED_RTSP" "$DETECTOR_ENABLED" "$LOG"
