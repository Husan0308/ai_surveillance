...
cd "$(dirname "$0")/.."
ROOT="$PWD"
OUT="${V11_STEP4_MATCH_ACCEPTANCE_OUT:-/tmp/camera_v11_step4_reid_same_room_matcher_v2}"
...
V11_STEP4_MATCH_RUN_SEC="$DURATION" \
  ...
  bash "$ROOT/scripts/run_camera_v11_step4_reid_same_room_matcher_v2.sh" \
  >"$LAUNCHER_LOG" 2>&1
status=$?
...
grep -q '^CAMERA_V11_STEP4_REID_MATCH_POWER_PRIME phase=gui_held result=PASS ' "$LAUNCHER_LOG" \
  || fail "no_successful_active_prime"
grep -q '^CAMERA_V11_STEP4_REID_MATCH_PREFLIGHT result=PASS ' "$LAUNCHER_LOG" \
  || fail "no_preflight_pass"
grep -q '^CAMERA_V11_POWERMIZER_KEEPER result=PRIME_DIAGNOSTIC ' "$LAUNCHER_LOG" \
  || fail "no_power_diagnostic"
