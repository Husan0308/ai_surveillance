#!/usr/bin/env python3
"""Fail closed on weak CAM-01 + CAM-02 + CAM-03 DeepStream 9.1 soak evidence."""
import argparse
import csv
import json
...
def assess(source_rows, group_rows, text, gpu):
    failures = []
    if set(source_rows) != {"CAM-01", "CAM-02", "CAM-03"}:
    ...
    if "nvstreammux(batch=3" not in text or "inference=0" not in text:
        failures.append("Missing three-source mux/no-inference evidence")
...
def main() -> int:
    ...
    probe = subprocess.run(
        [
            ...
            "-of",
            "json",
            str(args.directory / "CAM-01_CAM-02_CAM-03.mkv"),
        ],
        capture_output=True,
        text=True,
        timeout=60,
