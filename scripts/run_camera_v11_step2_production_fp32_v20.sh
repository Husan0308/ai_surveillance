...
LOCK_FILE="/tmp/ai_surveillance_camera_v11_step2_production_v20.lock"
...
fail() {
  printf 'CAMERA_V11_STEP2_V20_PREFLIGHT ERROR: %s\n' "$*" >&2
  exit 1
}
...
: >"$DISPLAY_LOG"
: >"$DETECTOR_LOG"
printf 'CAMERA_V11_STEP2_V20_PREFLIGHT status=OK base=%s precision=fp32 display_log=%s detector_log=%s\n' \
...
detector_pid=$!
printf 'CAMERA_V11_STEP2_V20_RUNNING display_pid=%s detector_pid=%s\n' "$display_pid" "$detector_pid"
