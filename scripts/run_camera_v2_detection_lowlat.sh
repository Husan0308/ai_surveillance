...
# Never carry diagnostic parity mode into production.
...
MODULE="services.camera_v2.detection_lowlat"

fail() { printf 'CAMERA_LOWLAT_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
if not str(trt.__version__).startswith('8.6.1'):
    raise SystemExit(f'CAMERA_LOWLAT_PREFLIGHT ERROR: TensorRT 8.6.1 required, got {trt.__version__}')
print(f'CAMERA_LOWLAT_TRT tensorrt={trt.__version__}', flush=True)
PY

if [[ "$CAMERA_V2_RESCUE_ENABLED" == "1" ]]; then
  if [[ ! -s "$CAMERA_V2_RESCUE_TRT86_ENGINE" ]]; then
    echo "CAMERA_LOWLAT_RESCUE engine_missing=1 action=prepare_m672" >&2
    if ! bash "$RESCUE_PREP"; then
      echo "CAMERA_LOWLAT_RESCUE WARNING prepare failed; continuing primary-only" >&2
      export CAMERA_V2_RESCUE_ENABLED=0
    fi
  fi
  if [[ "$CAMERA_V2_RESCUE_ENABLED" == "1" && ! -s "$CAMERA_V2_RESCUE_TRT86_ENGINE" ]]; then
    echo "CAMERA_LOWLAT_RESCUE WARNING engine still missing; continuing primary-only" >&2
    export CAMERA_V2_RESCUE_ENABLED=0
  fi
fi
...
import services.camera_v2.detection_lowlat  # noqa: F401
PY
  then MAIN_PYTHON="$candidate"; break; fi
done
[[ -n "$MAIN_PYTHON" ]] || fail "no Python can import low-latency runtime"

printf '%s\n' \
  "CAMERA_LOWLAT_PROFILE source=6xRTSP@20 rtsp=TCP/${CAMERA_V2_RTSP_LATENCY_MS}ms mux=${CAMERA_V2_FRAME_WIDTH}x${CAMERA_V2_FRAME_HEIGHT} wall=${CAMERA_V2_WALL_WIDTH}x${CAMERA_V2_WALL_HEIGHT}" \
  "CAMERA_LOWLAT_PRIMARY model=YOLO26s engine=TRT8.6/672x384/B1 target=${CAMERA_V2_DETECT_TARGET_HZ}Hz/cam adaptive=${CAMERA_V2_PRIMARY_HZ_MIN}-${CAMERA_V2_PRIMARY_HZ_MAX} conf=${CAMERA_V2_DETECT_CONF}" \
  "CAMERA_LOWLAT_RESCUE enabled=${CAMERA_V2_RESCUE_ENABLED} camera=${CAMERA_V2_RESCUE_CAMERA} model=YOLO26m engine=TRT8.6/672x384/B1 same_frame=1 duty=${CAMERA_V2_RESCUE_GPU_DUTY} min_interval=${CAMERA_V2_RESCUE_MIN_INTERVAL_SEC}s" \
  "CAMERA_LOWLAT_FLOW display=clean-wall ML=isolated-tee gate=preconvert capture_after_gpu_slot=1 nvdcf=0 osd=0 nms=0 dedup=0" \
  "CAMERA_LOWLAT_MAIN executable=$MAIN_PYTHON module=$MODULE"
...
while true; do
  ...
  echo "CAMERA_LOWLAT_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
