...
PY="${CAMERA_V107_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V107_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
for plugin in nvurisrcbin rtspsrc tee queue nvstreammux nvmultistreamtiler nvvideoconvert appsink nvtracker nvdsosd nveglglessink fakesink; do
...
Gst.init(None)
rtspsrc = Gst.ElementFactory.make("rtspsrc", "v107_rtspsrc_probe")
tcp_ts = int(rtspsrc is not None and rtspsrc.find_property("tcp-timestamp") is not None)
import tensorrt as trt
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v107_cam02_latency120 import PascalCam02Latency120Runtime  # noqa
print(f"CAMERA_V107_IMPORT python={sys.version.split()[0]} trt={trt.__version__} gst={Gst.version_string()} tcp_timestamp_property={tcp_ts} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V107_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V107_PROFILE behavior=v105-plus-cam02-latency120 tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V107_POLICY only_change=CAM-02-rtsp-latency-60-to-120 global_rtsp_latency_ms=${CAMERA_V2_RTSP_LATENCY_MS} batch_size=6 timeout_us=60000 pre_mux_gate=off post_mux_gate=10Hz sync_inputs=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v107_cam02_latency120
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V107_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
