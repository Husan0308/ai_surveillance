...
PY="${CAMERA_V95_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V95_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
# V9.5 is diagnostic-only over V9.4. Keep every V9.4 behavior setting identical.
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v95_pts_audit import PascalPtsAuditRuntime  # noqa
print(f"CAMERA_V95_IMPORT python={sys.version.split()[0]} trt={trt.__version__} gi=OK runtime=OK")
PY
then
  fail "Python 3.10 V9.5 runtime import failed"
fi

printf '%s\n' \
  "CAMERA_V95_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V95_PROFILE behavior=v94-unchanged source=${CAMERA_V2_SOURCE_FPS}fps tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V95_POLICY only_change=PTS-instrumentation NvDsFrameMeta.buf_pts=1 bbox_behavior_change=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v95_pts_audit
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V95_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
