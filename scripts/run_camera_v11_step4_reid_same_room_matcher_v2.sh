...
# Frozen helper is sourced read-only. Step1/2/3 files remain unchanged.
...
printf 'CAMERA_V11_STEP4_REID_MATCH_POWER_PRIME phase=baseline result=PASS\n'

v11_powermizer_start || fail "powermizer_keeper_start"
sleep 1
printf 'CAMERA_V11_STEP4_REID_MATCH_POWER_PRIME phase=gui_held start=1 monitor=active_workload clock_gate=diagnostic_only\n'

# Important: on this Pascal/driver combination a successful CUDA/TensorRT workload
# can leave the memory clock at the 405 MHz idle domain. Do not use one clock
# domain as a proxy for end-to-end inference health. We still monitor P-state,
# SM clock, memory clock and utilization during the exact production prime, but
# authoritative acceptance remains the frozen Step2 throughput/result-age gates
# plus Step3/Step4 latency and display regression checks.
"$TRT_PY" "$PRIME_SCRIPT" --engine "$DETECTOR_ENGINE" --warmup 30 --iterations 100 \
...
while kill -0 "$prime_pid" 2>/dev/null; do
  ...
  if [[ -n "$sample" ]]; then
    ...
    if [[ "$pstate" =~ ^[0-9]+$ ]] && (( pstate < best_pstate )); then best_pstate=$pstate; fi
    if [[ "$sm" =~ ^[0-9]+$ ]] && (( sm > peak_sm )); then peak_sm=$sm; fi
    if [[ "$mem" =~ ^[0-9]+$ ]] && (( mem > peak_mem )); then peak_mem=$mem; fi
    if [[ "$util" =~ ^[0-9]+$ ]] && (( util > peak_util )); then peak_util=$util; fi
    if [[ "$util" =~ ^[0-9]+$ ]] && (( util > 0 )); then
      [[ "$mem" =~ ^[0-9]+$ ]] && last_active_mem=$mem
      [[ "$sm" =~ ^[0-9]+$ ]] && last_active_sm=$sm
    fi
    samples=$((samples + 1))
  fi
  sleep 0.02
done
wait "$prime_pid" || fail "power_prime_gui_held_failed"
prime_pid=""
post_mem="$(v11_powermizer_mem_clock_mhz || true)"
post_pstate="$(nvidia-smi --query-gpu=pstate --format=csv,noheader 2>/dev/null | head -n 1 | tr -d '[:space:]' || true)"
(( best_pstate == 99 )) && best_pstate=-1
printf 'CAMERA_V11_POWERMIZER_KEEPER result=PRIME_DIAGNOSTIC peak_memory_mhz=%s peak_sm_mhz=%s peak_gpu_util=%s best_pstate=P%s last_active_memory_mhz=%s last_active_sm_mhz=%s post_memory_mhz=%s post_pstate=%s samples=%s memory_gate=diagnostic_only\n' \
  "$peak_mem" "$peak_sm" "$peak_util" "$best_pstate" "$last_active_mem" "$last_active_sm" "${post_mem:-unknown}" "${post_pstate:-unknown}" "$samples"
printf 'CAMERA_V11_STEP4_REID_MATCH_POWER_PRIME phase=gui_held result=PASS benchmark_exit=0 power_acceptance=runtime_metrics\n'
printf 'CAMERA_V11_STEP4_REID_MATCH_PREFLIGHT result=PASS frozen_sha=%s reid_engine=%s precision=fp32 pair_tsv=%s match_tsv=%s power_clock_gate=diagnostic_only runtime_regression_gate=authoritative\n' \
...
runtime_clock="$(v11_powermizer_mem_clock_mhz || true)"
runtime_pstate="$(nvidia-smi --query-gpu=pstate --format=csv,noheader 2>/dev/null | head -n 1 | tr -d '[:space:]' || true)"
printf 'CAMERA_V11_STEP4_REID_MATCH_RUNTIME_CLOCK memory_mhz=%s pstate=%s diagnostic_only=1\n' \
  "${runtime_clock:-unknown}" "${runtime_pstate:-unknown}"
