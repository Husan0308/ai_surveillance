...
export CAMERA_V2_TRT86_SHM_WORKER="${CAMERA_V2_TRT86_SHM_WORKER:-$ROOT/scripts/yolo26_trt86_shm_worker_v2.py}"
...
import services.camera_v2.person_tracking_trt86_fresh  # noqa: F401
PY
  then
...
[[ -n "$MAIN_PYTHON" ]] || fail "no Python can import Camera V2 fresh runtime"
...
printf '%s\n' \
  ...
  "CAM01_TRT86_PIPELINE backend=trt86-sidecar-shm-bgr-v2 capture=jit-latest no_prefetch=1 appsink_async=0 queue_depth=1 nvdcf=per-frame" \
  "CAM01_TRT86_MAIN_PYTHON executable=$MAIN_PYTHON"

exec "$MAIN_PYTHON" -u -m services.camera_v2.person_tracking_trt86_fresh
