#!/usr/bin/env python3
"""Fail closed on weak CAM-01 + CAM-02 + CAM-03 + CAM-04 + CAM-05 + CAM-06 DeepStream 9.1 soak evidence."""
import argparse
import csv
import json
...
def assess(source_rows, group_rows, text, gpu):
    failures = []
    if set(source_rows) != {"CAM-01", "CAM-02", "CAM-03", "CAM-04", "CAM-05", "CAM-06"}:
    ...
    stable_group = [r for r in group_rows if r.get("elapsed", 0) >= 30]
    if len(stable_group) < 2:
        failures.append("GROUP: insufficient telemetry")
    else:
        if stable_group[-1]["elapsed"] - stable_group[0]["elapsed"] < 600:
            failures.append("GROUP: less than 600 seconds after warmup")
        for a, b in zip(stable_group, stable_group[1:]):
        ...
        if missing_pair_fields:
            ...
            failures.append(f"GROUP: malformed telemetry missing required field(s): {preview}")
        ...
        if any(
            ...
            or r.get("pts_backwards", 0) > 0
            or r.get("pts_duplicates", 0) > 0
            for r in stable_group
        ):
            failures.append("GROUP: output/error/timestamp threshold exceeded")

        resource_rows = [r for r in stable_group if "rss_mib" in r and "cpu_pct" in r]
        if len(resource_rows) < len(stable_group):
            failures.append("GROUP: incomplete RSS/CPU telemetry")
        elif resource_rows:
            ...
            if rss_growth > 48:
                failures.append("GROUP: RSS grew more than 48 MiB")
            if cpu_mean > 50:
                failures.append("GROUP: CPU usage exceeded 50% of one logical core")
    if "nvstreammux(batch=6" not in text or "inference=0" not in text:
        failures.append("Missing six-source mux/no-inference evidence")
    ...
    if "GROUP END " not in text or "fatal=0" not in text:
        failures.append("Missing clean group completion")
    if re.search(r"GROUP FATAL|CRITICAL|MESA: error|libEGL warning|\bERROR\s", text):
        failures.append("Fatal/critical runtime diagnostic in log")

    g = []
    if group_rows:
        end = group_rows[-1].get("elapsed", 0)
    ...
    if stable_group:
        summary.update(
            {
                "measured_seconds": group_rows[-1]["elapsed"],
                "steady_seconds": stable_group[-1]["elapsed"] - stable_group[0]["elapsed"],
                "output_frames": int(group_rows[-1]["output"]),
                "group_fps_min": min(r["fps"] for r in stable_group),
                "group_fps_max": max(r["fps"] for r in stable_group),
                "group_fps_mean": statistics.mean(r["fps"] for r in stable_group),
                "cpu_pct_mean_one_core": (
                    statistics.mean(r["cpu_pct"] for r in stable_group if "cpu_pct" in r)
                    if any("cpu_pct" in r for r in stable_group)
                    else None
                ),
                "rss_mib_start": next((r["rss_mib"] for r in stable_group if "rss_mib" in r), None),
                "rss_mib_end": next((r["rss_mib"] for r in reversed(stable_group) if "rss_mib" in r), None),
            }
        )
    for cid, rows in source_rows.items():
        stable = [r for r in rows if r.get("elapsed", 0) >= 30]
...
def main() -> int:
    ...
    source_rows = {
        ...
        "CAM-03": parse_rows(text, "CAM-03 STATS "),
        "CAM-04": parse_rows(text, "CAM-04 STATS "),
        "CAM-05": parse_rows(text, "CAM-05 STATS "),
        "CAM-06": parse_rows(text, "CAM-06 STATS "),
    }
    group_rows = parse_rows(text, "GROUP STATS ")
    ...
    report = assess(source_rows, group_rows, text, gpu)
    ...
    probe = subprocess.run(
        [
            ...
            "-of",
            "json",
            str(args.directory / "CAM-01_CAM-02_CAM-03_CAM-04_CAM-05_CAM-06.mkv"),
        ],
        capture_output=True,
        text=True,
        timeout=60,
    ...
    try:
        ...
        if stream["width"] != 1920 or stream["height"] != 1620 or stream["codec_name"] != "h264":
            raise ValueError("unexpected output format")
        if int(stream["nb_read_packets"]) < 12000:
            raise ValueError("recording has too few frames")
        if group_rows and abs(int(stream["nb_read_packets"]) - int(group_rows[-1]["output"])) > 16:
