...
# Single A/B change versus V8.5: tracker width 512 -> 480. Height/cadence stay fixed.
export CAMERA_V2_TRACK_WIDTH="${CAMERA_V2_TRACK_WIDTH:-480}"
...
# Exact V8.5 batch-1 detector policy.
...
Gst.init(None)
from services.camera_v2.runtime_v87_tracker480 import PascalTracker480Runtime  # noqa
PY
  then MAIN_PYTHON="$candidate"; break; fi
done
[[ -n "$MAIN_PYTHON" ]] || fail "no Python can import V8.7 runtime"

export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"
printf '%s\n' \
  "CAMERA_V87_PREFLIGHT status=OK python=$MAIN_PYTHON single_owner=1" \
  "CAMERA_V87_PROFILE source=${CAMERA_V2_SOURCE_FPS}fps tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz feature_level=${CAMERA_V85_NVDCF_FEATURE_LEVEL} detector=batch1-v84" \
  "CAMERA_V87_POLICY only_change=tracker-resolution-512x288-to-480x288 blocking_sync_override=0 bbox_unchanged=1 detector_unchanged=1 cadence_unchanged=1 confidence_unchanged=1"

restart_count=0
while true; do
  set +e
  "$MAIN_PYTHON" -u -m services.camera_v2.runtime_v87_tracker480
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V87_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
