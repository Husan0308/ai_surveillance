...
# Exact restored architecture: local NvDCF remains authoritative; Global ID is
# an isolated ReID side path that only replaces the text/style of existing tracks.
...
[[ -x "$PY" ]] || { echo "RESTORE_GLOBAL_ID ERROR missing python: $PY" >&2; exit 1; }

printf '%s\n' \
  "RESTORE_GLOBAL_ID_PROFILE local=NvDCF detector=YOLO26m/736x416/b2 global=async-ReID" \
  "RESTORE_GLOBAL_ID_POLICY bbox_owner=NvDCF global_id=label-only qwen=off" \
  "RESTORE_GLOBAL_ID_LABEL confirmed=G### tentative=G###? conflict=G###!"

exec "$PY" -u -m services.camera_v2.person_tracking_reid
