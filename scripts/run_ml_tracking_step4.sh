...
# Frozen Step 3 detector defaults.
...
# Step 4 local tracking defaults. All are CPU-only and per-camera.
...
export ML_TRACK_SHADOW_SEC="${ML_TRACK_SHADOW_SEC:-0.9}"
export ML_TRACK_MAX_LOST_SEC="${ML_TRACK_MAX_LOST_SEC:-2.5}"
export ML_TRACK_APPEARANCE_WEIGHT="${ML_TRACK_APPEARANCE_WEIGHT:-0.18}"
...
fail() { printf 'ML_STEP4_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(f"ML_STEP4_PREFLIGHT ERROR: TensorRT 8.6.1 required, got {trt.__version__}")
if sys.prefix == sys.base_prefix:
    raise SystemExit(
        "ML_STEP4_PREFLIGHT ERROR: TRT86 interpreter is not inside venv: "
...
from services.ml_service.app.local_tracker import LocalPersonTracker  # noqa: F401
from services.ml_service.app.detector_substream_tracking import DetectorSubstreamTrackingService  # noqa: F401
print(
    "ML_STEP4_IMPORTS status=OK detector_v14_frozen=1 cpu_tracker=1 "
    "time_aware=1 two_stage_assoc=1 model_free_appearance=1 gpu_tracker=0",
    flush=True,
)
PY

printf '%s\n' \
  "ML_STEP4_PROFILE detector=V14-token3 target=${ML_DETECTOR_TARGET_HZ}Hz/cam rtsp=${ML_SUBSTREAM_RTSP_LATENCY_MS}ms pending_depth=${ML_SUBSTREAM_PENDING_DEPTH}" \
  "ML_STEP4_PROFILE tracker=CPU-time-aware-bytetrack-style low=${ML_TRACK_LOW_THRESH} high=${ML_TRACK_HIGH_THRESH} new=${ML_TRACK_NEW_THRESH} max_lost=${ML_TRACK_MAX_LOST_SEC}s shadow=${ML_TRACK_SHADOW_SEC}s appearance_weight=${ML_TRACK_APPEARANCE_WEIGHT}" \
  "ML_STEP4_BOUNDARY camera_service=independent main_stream=0 nvdcf=0 gpu_tracker=0 reid=0 global_id=0 api=0 ui=0 overlay_metadata=normalized-box-plus-velocity"

exec "$MAIN_PYTHON" -u -m services.ml_service.app.detector_substream_tracking
