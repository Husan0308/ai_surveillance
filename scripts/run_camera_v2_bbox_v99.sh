...
PY="${CAMERA_V99_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V99_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
# V9.9 is diagnostic-only over V9.8. Keep every runtime behavior identical.
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v99_tracker_latency_audit import PascalTrackerLatencyAuditRuntime  # noqa
print(f"CAMERA_V99_IMPORT python={sys.version.split()[0]} trt={trt.__version__} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V99_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V99_PROFILE behavior=v98-unchanged display_pool=4 tracker_pool=4 tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V99_POLICY only_change=tracker-stage-PTS-instrumentation bbox_behavior_change=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v99_tracker_latency_audit
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V99_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
