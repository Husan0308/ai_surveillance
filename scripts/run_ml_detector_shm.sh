...
export ML_DETECTOR_CAPTURE_TIMEOUT_MS="${ML_DETECTOR_CAPTURE_TIMEOUT_MS:-300}"
export ML_DETECTOR_MAX_INPUT_AGE_MS="${ML_DETECTOR_MAX_INPUT_AGE_MS:-200}"
...
printf '%s\n' \
  "ML_DETECTOR_ARCH camera_rtsp=0 camera_nvdec=0 deepstream=0 tracker=0 api=0 ui=0 input=SHM-demand-jit output=detections-only" \
  "ML_DETECTOR_INPUT dir=$ML_DETECTOR_SHM_DIR cameras=$ML_DETECTOR_CAMERAS target=${ML_DETECTOR_TARGET_HZ}Hz/cam geometry=672x378x3 policy=request-next-frame" \
