...
PY="${CAMERA_V101_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V101_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v101_tracker_batch_audit import PascalTrackerBatchAuditRuntime  # noqa
print(f"CAMERA_V101_IMPORT python={sys.version.split()[0]} trt={trt.__version__} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V101_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V101_PROFILE behavior=v100-unchanged tracker_timeout_us=40000 display_pool=4 tracker_pool=4 tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V101_POLICY only_change=tracker-batch-formation-instrumentation bbox_behavior_change=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v101_tracker_batch_audit
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V101_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
