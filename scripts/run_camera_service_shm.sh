...
for plugin in nvurisrcbin tee queue nvstreammux nvmultistreamtiler nvvideoconvert capsfilter appsink nveglglessink; do
...
printf '%s\n' \
  "CAMERA_SERVICE_SHM_PROFILE source=${CAMERA_SERVICE_SOURCE_FPS}fps rtsp=${CAMERA_SERVICE_RTSP_LATENCY_MS}ms display=${CAMERA_SERVICE_DISPLAY_WIDTH}x${CAMERA_SERVICE_DISPLAY_HEIGHT}" \
  ...
  "CAMERA_SERVICE_SHM_INVARIANT display_never_waits_for_consumer=1 latest_only=1 gate_before_convert=1"
