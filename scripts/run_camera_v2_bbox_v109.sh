...
PY="${CAMERA_V109_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V109_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
export CAMERA_V2_RTSP_LATENCY_MS="60"
...
for plugin in nvurisrcbin rtspsrc rtpbin rtpjitterbuffer tee queue nvstreammux nvmultistreamtiler nvvideoconvert appsink nvtracker nvdsosd nveglglessink fakesink; do
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v109_rtp_health_audit import PascalRtpHealthAuditRuntime  # noqa
print(f"CAMERA_V109_IMPORT python={sys.version.split()[0]} trt={trt.__version__} gst={Gst.version_string()} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V109_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V109_PROFILE behavior=v105-v104-baseline-plus-rtp-audit tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V109_POLICY behavior_change=0 transport=tcp rtsp_latency_ms=60 batch_size=6 timeout_us=60000 pre_mux_gate=off post_mux_gate=10Hz sync_inputs=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v109_rtp_health_audit
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V109_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
