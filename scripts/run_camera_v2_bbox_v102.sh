...
PY="${CAMERA_V102_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V102_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
# Keep V10.0/V10.1 behavior; only tracker_mux batch-size changes 6 -> 4.
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v102_tracker_batch4 import PascalTrackerBatch4Runtime  # noqa
print(f"CAMERA_V102_IMPORT python={sys.version.split()[0]} trt={trt.__version__} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V102_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V102_PROFILE behavior=v101-plus-batch4 display_pool=4 tracker_pool=4 tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V102_POLICY only_change=tracker-mux-batch-size-6-to-4 timeout_us=40000"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v102_tracker_batch4
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V102_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
