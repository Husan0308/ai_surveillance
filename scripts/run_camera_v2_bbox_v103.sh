...
PY="${CAMERA_V103_PYTHON:-$ROOT/.venv-trt86/bin/python}"

fail() { printf 'CAMERA_V103_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
...
# V10.3 rolls back the failed V10.2 batch-size=4 A/B and keeps V10.0's
# six-source mux target + 40 ms timeout.  The only new behavior is a shared
# monotonic 10 Hz phase for all six pre-mux tracker gates.
...
if not str(trt.__version__).startswith("8.6.1"):
    raise SystemExit(11)
from services.camera_v2.runtime_v103_shared_phase_gate import PascalTrackerSharedPhaseRuntime  # noqa
print(f"CAMERA_V103_IMPORT python={sys.version.split()[0]} trt={trt.__version__} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V103_PREFLIGHT status=OK python=$PY single_owner=1" \
  "CAMERA_V103_PROFILE behavior=v100-plus-shared-phase display_pool=4 tracker_pool=4 tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz" \
  "CAMERA_V103_POLICY only_change=tracker-gate-independent-to-shared-monotonic-phase batch_size=6 timeout_us=40000 sync_inputs=0"

restart_count=0
while true; do
  set +e
  "$PY" -u -m services.camera_v2.runtime_v103_shared_phase_gate
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V103_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
