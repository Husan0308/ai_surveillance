...
for plugin in nvurisrcbin queue tee nvstreammux nvvideoconvert capsfilter appsink nvdsosd nveglglessink rtspsrc; do
...
# Fail early if this is not the isolated TensorRT 8.6 environment.
TRT_VERSION="$($TRT_PY -I -c 'import tensorrt as trt; print(trt.__version__)' 2>/dev/null || true)"
[[ "$TRT_VERSION" == 8.6.1* ]] || fail "trt86_python_bad_version=${TRT_VERSION:-missing}"
...
export V11_DS_YOLO_BOX_STALE_SEC="${V11_DS_YOLO_BOX_STALE_SEC:-1.10}"
...
printf 'V11_DS_YOLO_CAM01_GPU_POLICY gst_nvinfer=0 trt10=0 trt86=1 second_rtsp=0 model_pt_required=0 parser_build=0 log=%s\n' "$LOG"

: >"$LOG"
"$APP_PY" -u -m services.camera_v11.deepstream_trt86_cam01_v2 2>&1 | tee "$LOG"
