...
def main() -> int:
    ...
    ap.add_argument("--log", default="/tmp/CAMERA_V11_BBOX_NVDCF_CAM01_CAM02.log")
    ap.add_argument("--required-cameras", default="CAM-01,CAM-02")
    ...
    path = Path(args.log)
    if not path.is_file():
        print(f"V11_BBOX_NVDCF_CAM01_CAM02_CHECK RESULT=FAIL reasons=log_missing:{path}")
        return 1
    text = path.read_text(encoding="utf-8", errors="replace")
    reasons: list[str] = []

    if "CAMERA_V11_BBOX_NVDCF_ARCH cameras=CAM-01,CAM-02" not in text:
    ...
    if reasons:
        print(
            "V11_BBOX_NVDCF_CAM01_CAM02_CHECK RESULT=FAIL reasons="
            + ";".join(reasons)
    ...
    print(
        "V11_BBOX_NVDCF_CAM01_CAM02_CHECK RESULT=PASS required="
        + ",".join(required)
        + " details="
        + " | ".join(details)
    )
