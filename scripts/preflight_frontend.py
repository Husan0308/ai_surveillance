...
def main() -> int:
    ...
    try:
        ...
        if not 10 <= settings.frame_refresh_interval_ms <= 1000:
            raise ValueError("FRONTEND_FRAME_REFRESH_INTERVAL_MS must be 10..1000")
    except Exception as exc:
    ...
    print(
        ...
        f"metadata_ms={settings.refresh_interval_ms} frame_ms={settings.frame_refresh_interval_ms}",
        flush=True,
    )
