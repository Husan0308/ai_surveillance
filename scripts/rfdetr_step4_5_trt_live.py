...
def _args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Step 4.5: tracker-free live RF-DETR-S TensorRT 8.6 FP32 detector. "
...
def _preload_cuda_stack() -> dict[str, str]:
    ...
    for name, patterns in specs.items():
        ...
        if path is None:
            raise SystemExit(f"STEP4_5_FAIL nvidia_library_missing={name}")
...
def _load_cudart() -> ctypes.CDLL:
    ...
    if path is None:
        raise SystemExit("STEP4_5_FAIL cudart_missing")
...
class TrtRunner:
    def __init__(self, engine_path: Path) -> None:
        ...
        if not str(trt.__version__).startswith("8.6.1"):
            raise SystemExit(f"STEP4_5_FAIL wrong_tensorrt={trt.__version__}")
        self.trt = trt
        ...
        if self.engine is None:
            raise SystemExit("STEP4_5_FAIL engine_deserialize")
        self.context = self.engine.create_execution_context()
        if self.context is None:
            raise SystemExit("STEP4_5_FAIL context_create")
        ...
        if self.input_index < 0 or self.input_shape is None or self.input_dtype is None:
            raise SystemExit("STEP4_5_FAIL input_binding_missing")
        if "dets" not in self.outputs or "labels" not in self.outputs:
            raise SystemExit(f"STEP4_5_FAIL output_bindings={list(self.outputs)}")
...
def _start_ffmpeg(args: argparse.Namespace):
    ...
    if camera is None:
        ...
        raise SystemExit(f"STEP4_5_FAIL camera_not_found={args.camera} available=[{available}]")
    ffmpeg = shutil.which("ffmpeg")
    if not ffmpeg:
        raise SystemExit("STEP4_5_FAIL ffmpeg_not_found")
...
def main() -> int:
    ...
    if args.seconds < 5.0 or args.seconds > 300.0:
        raise SystemExit("STEP4_5_FAIL seconds_must_be_5_to_300")
    if args.capture_fps <= 0.0 or args.capture_fps > 20.0:
        raise SystemExit("STEP4_5_FAIL capture_fps_must_be_0_to_20")
    if (args.width, args.height) != (800, 448):
        raise SystemExit(f"STEP4_5_FAIL engine_shape_fixed_800x448 got={args.width}x{args.height}")
    if not args.engine.is_file() or args.engine.stat().st_size == 0:
        raise SystemExit(f"STEP4_5_FAIL engine_not_found={args.engine}")
    ...
    if runner.input_shape != (1, 3, args.height, args.width):
        runner.close()
        raise SystemExit(f"STEP4_5_FAIL engine_input_shape={runner.input_shape}")
    ...
    if first is None:
        ...
        raise SystemExit(f"STEP4_5_FAIL live_start camera={args.camera} error={error[-500:]}")
    ...
    print(
        f"STEP4_5_LIVE camera={camera.camera_id} duration={args.seconds:.1f}s "
    ...
    try:
        while time.monotonic() < deadline:
            ...
            print(
                "STEP4_5_FRAME "
                f"n={processed:03d} seq={seq} raw={len(raw_persons)} unique={len(kept)} dup={len(rejected)} "
                f"skipped={skipped} age_ms={age_ms:.1f} prep_ms={prep_ms:.1f} "
            ...
            if args.save_every > 0 and (processed == 1 or processed % args.save_every == 0):
                footer = f"{camera.camera_id} n={processed} unique={len(kept)} trt={trt_ms:.0f}ms age={age_ms:.0f}ms"
                _annotate(raw, args.width, args.height, kept, footer).save(
    ...
    if processed == 0:
        ...
        raise SystemExit(f"STEP4_5_FAIL no_results error={error[-500:]}")
    ...
    print(f"STEP4_5_JSON={report_path}", flush=True)
    print(f"STEP4_5_SAMPLES={sample_dir}", flush=True)
    print("STEP4_5_PASS", flush=True)
    return 0
