...
def main() -> int:
    ...
    rows = [row for row in (camera_data.get("cameras") or []) if isinstance(row, dict) and row.get("enabled", True) is not False]
    ...
    for row in rows:
        ...
        if not uri.startswith("rtsp://"):
            fail(f"{cid} missing rtsp:// URI")

    cfg = dict(core_data.get("camera_core") or {})
    if int(cfg.get("queue_buffers", 0)) != 1:
        fail("queue_buffers must remain 1 for bounded live latency")
    if bool(cfg.get("sync_inputs", True)):
        fail("sync_inputs must be false so one camera cannot stall the others")
    if bool(cfg.get("sink_sync", True)):
        fail("sink_sync must be false for the live wall baseline")
    if bool(cfg.get("sink_qos", True)):
        fail("sink_qos must be false for the live wall baseline")
    ...
    auth = bool(os.environ.get("RTSP_USERNAME"))
    print(
        "VISION_V3_CAMERA_PREFLIGHT=PASS "
        f"cameras=6 auth={'env' if auth else 'yaml-or-none'} "
