...
LOCK_FILE="/tmp/ai_surveillance_camera_v11_step1_v8.lock"

fail() { printf 'CAMERA_V11_STEP1V8_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
command -v flock >/dev/null 2>&1 || fail "flock missing"
exec 9>"$LOCK_FILE"
flock -n 9 || fail "another V11 Step1 V8 runtime holds $LOCK_FILE; stop it first"
...
# Only CAM-02 scaling algorithm changes: 2 = GPU Cubic.
export V11_CAM02_INTERPOLATION="${V11_CAM02_INTERPOLATION:-2}"

"$PY" - <<'PY'
import sys
if sys.version_info[:2] != (3, 10):
    raise SystemExit(f"V11 Step1 V8 requires Python 3.10, got {sys.version}")
import gi
...
printf '%s\n' \
  "CAMERA_V11_STEP1V8_PREFLIGHT status=OK python=$PY display=${DISPLAY}" \
  "CAMERA_V11_STEP1V8_INVARIANT base=v4-ds100 mux=0 tiler=0 detector=0 tracker=0 latest_only=1 transport=tcp latency_ms=${V11_RTSP_LATENCY_MS}" \
  "CAMERA_V11_STEP1V8_AB cam02_interpolation=${V11_CAM02_INTERPOLATION} others_interpolation=${V11_SCALE_INTERPOLATION}"

exec "$PY" -u -m services.camera_v11.step1_cam02_cubic_v8
