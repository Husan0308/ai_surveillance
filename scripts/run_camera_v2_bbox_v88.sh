...
# V8.4 batch-1 detector unchanged.
...
# One V8.8 A/B change: give one TRT call a short exclusive admission window.
export CAMERA_V88_QUIET_BEFORE_MS="${CAMERA_V88_QUIET_BEFORE_MS:-30}"
...
Gst.init(None)
from services.camera_v2.runtime_v88_serialized_gpu import PascalSerializedGpuOwnershipRuntime  # noqa
PY
  then MAIN_PYTHON="$candidate"; break; fi
done
[[ -n "$MAIN_PYTHON" ]] || fail "no Python can import V8.8 runtime"

export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"
printf '%s\n' \
  "CAMERA_V88_PREFLIGHT status=OK python=$MAIN_PYTHON single_owner=1" \
  "CAMERA_V88_PROFILE source=${CAMERA_V2_SOURCE_FPS}fps tracker=${CAMERA_V2_TRACK_WIDTH}x${CAMERA_V2_TRACK_HEIGHT}@${CAMERA_V2_TRACK_FPS}Hz feature_level=${CAMERA_V85_NVDCF_FEATURE_LEVEL} detector=batch1-v84" \
  "CAMERA_V88_POLICY one_change=short-gpu-ownership quiet_before=${CAMERA_V88_QUIET_BEFORE_MS}ms display_gated=0 bbox_unchanged=1 mps=0"

restart_count=0
while true; do
  set +e
  "$MAIN_PYTHON" -u -m services.camera_v2.runtime_v88_serialized_gpu
  rc=$?
  set -e
  [[ $rc -eq 75 ]] || exit "$rc"
  ...
  delay=$restart_count
  (( delay > 10 )) && delay=10
  echo "CAMERA_V88_SUPERVISOR restart=$restart_count delay=${delay}s" >&2
  sleep "$delay"
done
