...
LOCK_FILE="/tmp/ai_surveillance_camera_v11_step2_sharedscale_v7.lock"
...
fail() { printf 'CAMERA_V11_STEP2V7_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
command -v flock >/dev/null 2>&1 || fail "flock missing"
exec 9>"$LOCK_FILE"
flock -n 9 || fail "another V11 Step2 V7 runtime holds $LOCK_FILE; stop it first"
...
# Step2 V7: keep batch1 at 2 Hz/camera; change only detector conversion topology.
...
if sys.version_info[:2] != (3, 10):
    raise SystemExit(f"V11 Step2 V7 requires Python 3.10, got {sys.version}")
import gi
gi.require_version("Gst", "1.0")
from gi.repository import Gst
Gst.init(None)
from services.camera_v11.batch1_trt86 import Batch1TRT86Client  # noqa: F401
from services.camera_v11.step2_detector_b1_sharedscale_v7 import V11Step2DetectorB1SharedScaleV7  # noqa: F401
print(f"CAMERA_V11_STEP2V7_IMPORT python={sys.version.split()[0]} gst={Gst.version_string()} runtime=OK")
PY
...
printf '%s\n' \
  ...
  "CAMERA_V11_STEP2V7_PREFLIGHT status=OK python=$PY display=${DISPLAY} engine=$ENGINE worker=$WORKER" \
  "CAMERA_V11_STEP2V7_INVARIANT base=step2-v6 display_geometry=640x360 batch=1 per_camera=2Hz shared_scale=1 fullres_detector_resize=0 latest_only=1 tracker=0 osd=0 reid=0 face=0" \
  "CAMERA_V11_STEP2V7_TARGET display_must_pass=1 global_detector=12Hz trt_p95_target=45ms conversion_p95_target=35ms result_age_target=140ms"

exec "$PY" -u -m services.camera_v11.step2_detector_b1_sharedscale_v7
