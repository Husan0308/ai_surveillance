...
DISPLAY_RUNNER="${V11_STEP4_DISPLAY_RUNNER:-$ROOT/scripts/run_camera_v11_step1_v7.sh}"
...
[[ -f "$DISPLAY_RUNNER" ]] || fail "display_runner_missing path=$DISPLAY_RUNNER"
...
CONFLICT_PATTERN='services\.camera_v11\.(step1_cam02_lowlat_v7|step4_display_jitter_ab_v1|step2_production_fp32(_v[0-9]+)?|step3_tracking_v[0-9]+|step4_tracking_reid_v[0-9]+)|yolo26_trt86_step2_worker\.py|reid_trt86_worker_v11\.py'
...
printf 'CAMERA_V11_STEP4_PREFLIGHT result=PASS frozen_step3_sha=%s runtime=trt86 reid_engine=1 display_log=%s step4_log=%s gpu_log=%s display_runner=%s\n' \
  "$FROZEN_STEP3_SHA" "$DISPLAY_LOG" "$STEP4_LOG" "$GPU_LOG" "$DISPLAY_RUNNER"

bash "$DISPLAY_RUNNER" >"$DISPLAY_LOG" 2>&1 &
display_pid=$!
...
printf 'CAMERA_V11_STEP4_RUNNING display_pid=%s step4_pid=%s telemetry_pid=%s keeper_pid=%s shadow_merge=0 quality_gate=1 diversity_gate=1 display_runner=%s\n' \
  "$display_pid" "$step4_pid" "$telemetry_pid" "$V11_POWERMIZER_KEEPER_PID" "$DISPLAY_RUNNER"
