...
fail() { printf 'CAMERA_V11_STEP1_PREFLIGHT ERROR: %s\n' "$*" >&2; exit 1; }
command -v flock >/dev/null 2>&1 || fail "flock missing"
exec 9>"$LOCK_FILE"
flock -n 9 || fail "another camera runtime holds $LOCK_FILE; stop it first"
...
if sys.version_info[:2] != (3, 10):
    raise SystemExit(f"V11 Step1 requires Python 3.10, got {sys.version}")
import gi
...
path = ensure_bridge()
from services.camera_v11.step1_display import V11Step1Display  # noqa: F401
print(f"CAMERA_V11_STEP1_IMPORT python={sys.version.split()[0]} gst={Gst.version_string()} pts_bridge={path} runtime=OK")
PY

printf '%s\n' \
  "CAMERA_V11_STEP1_PREFLIGHT status=OK python=$PY single_owner=1 display=${DISPLAY}" \
  "CAMERA_V11_STEP1_INVARIANT step0_ingest=frozen tracker=0 detector=0 osd=0 latest_only=1" \
  "CAMERA_V11_STEP1_PROFILE purpose=display-freshness tile=640x360 wall=1920x720 timeout_us=50000 pool=4"

exec "$PY" -u -m services.camera_v11.step1_display
