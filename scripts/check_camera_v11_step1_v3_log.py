...
CAM = re.compile(
    r"CAMERA_V11_STEP1V3_CAMERA camera=(\S+) source_fps=([0-9.]+) wall_p95=([0-9.]+)ms "
    r"pts_p95=([0-9.]+)ms mux_age_p95=([0-9.]+)ms display_age_p95=([0-9.]+)ms "
    r"mux_gap_p95=([0-9.]+)ms mux_samples=(\d+) display_samples=(\d+) pts_match_miss=(\d+) "
    r"input_q=(\d+) input_qmax=(\d+) errors=(\d+) warnings=(\d+)"
)
DISPLAY = re.compile(
    r"CAMERA_V11_STEP1V3_DISPLAY batch_fps=([0-9.]+) render_fps=([0-9.]+) "
    r"batch_size_p50=([0-9.]+) batch_size_p95=([0-9.]+) full_pct=([0-9.]+) "
    r"batch_q=(\d+) batch_qmax=(\d+) render_q=(\d+) render_qmax=(\d+) batches=(\d+)"
)
QUALITY = re.compile(
    r"CAMERA_V11_STEP1V3_QUALITY interpolation=(\d+) gpu_scaling=(\d+) single_resize=(\d+) "
    r"jpeg=(\d+) main_streams=(\d+) tile=(\d+)x(\d+) sync_inputs=(\d+) max_latency_ns=(\d+)"
)


def main() -> int:
    if len(sys.argv) != 2:
        print("usage: check_camera_v11_step1_v3_log.py /tmp/CAMERA_V11_STEP1V3.log")
        return 2

    text = Path(sys.argv[1]).read_text(encoding="utf-8", errors="replace")
    for marker in (
        "CAMERA_V11_STEP1V3_ARCH",
        "CAMERA_V11_STEP1V3_POLICY",
        "CAMERA_V11_STEP1V3_INVARIANT",
        "CAMERA_V11_STEP1V3_QUALITY",
    ):
        if marker not in text:
            print(f"V11_STEP1V3 FAIL missing={marker}")
            return 2
    ...
    if len(latest_cam) != 6 or not displays or not qualities:
        print(
            f"V11_STEP1V3 FAIL cameras={len(latest_cam)} "
    ...
    sync_inputs = int(q.group(8))
    max_latency_ns = int(q.group(9))
    ...
    if sync_inputs != 1:
        reasons.append(f"mux:sync_inputs={sync_inputs}")
    if not 55_000_000 <= max_latency_ns <= 250_000_000:
        reasons.append(f"mux:max_latency_ns={max_latency_ns}")
    ...
    print(
        ...
        f"sync_inputs={sync_inputs} max_latency_ns={max_latency_ns}"
    )

    if reasons:
        print("V11_STEP1V3 RESULT diagnosis=FAIL_DISPLAY_V3 reasons=" + ";".join(reasons))
        print("V11_STEP1V3 next=do not add detector/tracker; keep Step0 frozen and fix only V3 display timing")
        return 1

    print("V11_STEP1V3 RESULT diagnosis=PASS_DISPLAY_FRESH")
    print("V11_STEP1V3 next=freeze Step1; then create detector-only Step2 without changing ingest/display")
    return 0
