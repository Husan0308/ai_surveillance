...
cd "$(dirname "$0")/.."
ROOT="$PWD"
BRANCH_EXPECTED="rebuild/service-architecture-v11-bbox-nvdcf-cam01-cam02-v1-20260902"
LOG="${V11_BBOX_NVDCF_LOG:-/tmp/CAMERA_V11_BBOX_NVDCF_CAM01_CAM02.log}"
APP_PY="${V11_DS_YOLO_PYTHON:-$HOME/ai_surveillance/.venv/bin/python}"
[[ -x "$APP_PY" ]] || APP_PY="$(command -v python3)"
fail(){ printf 'V11_BBOX_NVDCF_CAM01_CAM02_PREFLIGHT RESULT=FAIL reason=%s\n' "$*" >&2; exit 1; }
...
export V11_DS_YOLO_CAMERAS="CAM-01,CAM-02"
export V11_UI_PREVIEW_CAMERAS="CAM-01,CAM-02"
...
export V11_BBOX_TRACK_CAMERAS="CAM-01,CAM-02"
# Reuse the CAM-01 profile that was physically accepted as good.  CAM-02 gets
# the exact same starting profile; tune CAM-02 only after live evidence.
...
"$APP_PY" -c 'from services.ml_service.app.config import load_settings; rows=load_settings().cameras; wanted={"CAM-01","CAM-02"}; found={c.camera_id for c in rows if c.username and c.password}; assert wanted <= found' || fail "camera_credentials_unresolved"
printf 'V11_BBOX_NVDCF_CAM01_CAM02_PREFLIGHT RESULT=PASS branch=%s cameras=CAM-01,CAM-02 rtsp_sources=2 detector_workers=1 detector_hz=%s raw_conf=%s tracker=nvdcf tracker_size=%sx%s reid=0 display_margin=0/0/0 profile=responsive-dynamic config=%s trt=%s log=%s\n' "$BRANCH_EXPECTED" "$V11_DS_YOLO_HZ" "$V11_DS_YOLO_CONF" "$V11_BBOX_TRACKER_WIDTH" "$V11_BBOX_TRACKER_HEIGHT" "$TRACKER_CFG" "$TRT_VERSION" "$LOG"
: >"$LOG"
