...
class Track:
    ...
    def advance_prediction(self):
        """Keep box stationary on missing frames; decay velocity to prevent drifting into space."""
        self.vx *= 0.5
        self.vy *= 0.5
...
class ByteTracker:
    def __init__(self,track_buffer=30,match_thresh=0.25,high_thresh=0.35,
        ...
        # Only return currently detected tracks (missing <= 1) to UI — prevents ghost boxes drifting/flying.
        self.visible_missing = 1
    def _iou(self,a,b):
        ...
        union=ua+ub-inter
        return inter/union if union>0 else 0.0
    def _match_score(self, tr, det):
        pred = tr.predict()
        ov = self._iou(pred, det)
        pcx = (pred[0] + pred[2]) * 0.5; pcy = (pred[1] + pred[3]) * 0.5
        dcx = (det[0] + det[2]) * 0.5; dcy = (det[1] + det[3]) * 0.5
        scale = max(20.0, pred[2] - pred[0], pred[3] - pred[1],
                    det[2] - det[0], det[3] - det[1])
        dist = ((pcx - dcx) ** 2 + (pcy - dcy) ** 2) ** 0.5 / scale
        # Center gating keeps a fast person attached even when IoU briefly drops.
        if ov < self.match_thresh and dist > 1.25:
            return None
        return max(ov, 1.0 - dist)
    ...
    def update(self,detections,det_embeddings=None):
        ...
        if not detections:
            for t in self.tracks:
                t.missing += 1
                t.advance_prediction()
            self.tracks=[t for t in self.tracks if t.missing<=self.track_buffer*2]
            return [t for t in self.tracks if t.missing<=self.visible_missing]
        high_dets,low_dets=[],[]
        for d in detections:
        ...
        if active and high_dets:
            ...
            for ti,tr in enumerate(active):
                for di,det in enumerate(high_dets):
                    ...
                    if s is not None: pairs.append((s,ti,di))
        ...
        if rem and low_dets:
            ...
            for ti,tr in enumerate(rem):
                for di,det in enumerate(low_dets):
                    s=self._match_score(tr,det)
                    if s is not None: p2.append((s,ti,di))
            p2.sort(key=lambda x:x[0],reverse=True); ml=set(); mdl=set()
            for s,ti,di in p2:
                if ti in ml or di in mdl: continue
                rem[ti].update(low_dets[di]); ml.add(ti); mdl.add(di)
        for di,det in enumerate(high_dets):
