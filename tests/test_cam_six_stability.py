...
from scripts.cam_six_validation.check_stability import assess, parse_rows


class SixCameraStabilityEvidenceTests(unittest.TestCase):
    def setUp(self):
        self.text = (
            "GROUP GRAPH nvstreammux(batch=6, inference=0\n"
            ...
            "CAM-06 END input=13200 hardware_decoder=1 errors=0 warnings=0\n"
            "GROUP END output=13200 fatal=0 shared_errors=0 shared_warnings=0"
        )
        self.sources = {"CAM-01": [], "CAM-02": [], "CAM-03": [], "CAM-04": [], "CAM-05": [], "CAM-06": []}
        self.group = []
        self.gpu = []
    ...
    def test_missing_camera_rejected(self):
        data = {"CAM-01": self.sources["CAM-01"], "CAM-02": self.sources["CAM-02"], "CAM-03": self.sources["CAM-03"], "CAM-04": self.sources["CAM-04"], "CAM-05": self.sources["CAM-05"]}
