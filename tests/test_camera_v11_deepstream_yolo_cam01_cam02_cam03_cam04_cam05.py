...
LAUNCHER = ROOT / "scripts/run_camera_v11_deepstream_yolo_cam01_cam02_cam03_cam04_cam05_v1.sh"
...
ARCH = (
    "CAMERA_V11_DS_YOLO_MULTI_ARCH cameras=5 camera_ids=CAM-01,CAM-02,CAM-03,CAM-04,CAM-05 "
    "rtsp_sources=5 rtsp_sessions=5 rtsp_per_camera=1 decode=deepstream-nvdec "
...
THREAD = "CAMERA_V11_DS_YOLO_MULTI_DETECTOR_THREAD state=START workers=1 cameras=5 scheduler=round-robin\n"
START = "CAMERA_V11_DS_YOLO_MULTI_START cameras=CAM-01,CAM-02,CAM-03,CAM-04,CAM-05 state=async\n"
...
def body(infer_cam05: float = 1.6) -> str:
...
def test_launcher_locks_exact_five_camera_increment() -> None:
    source = LAUNCHER.read_text()
    assert 'BRANCH_EXPECTED="rebuild/service-architecture-v11-deepstream-yolo-cam01-cam02-cam03-cam04-cam05-v1-20260901"' in source
    assert 'CAMERAS="${V11_DS_YOLO_CAMERAS:-CAM-01,CAM-02,CAM-03,CAM-04,CAM-05}"' in source
    assert 'camera_set=$CAMERAS expected=CAM-01,CAM-02,CAM-03,CAM-04,CAM-05' in source
...
def test_checker_accepts_five_fair_cameras(tmp_path: Path) -> None:
    result = run_checker(tmp_path, body())
    assert result.returncode == 0, result.stdout + result.stderr
    assert "RESULT=PASS" in result.stdout


def test_checker_rejects_cam05_starvation(tmp_path: Path) -> None:
    result = run_checker(tmp_path, body(infer_cam05=0.5))
    assert result.returncode == 1
    assert "CAM-05:infer_hz_min=0.50<1.20" in result.stdout
