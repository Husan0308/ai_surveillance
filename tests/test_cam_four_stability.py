...
from scripts.cam_four_validation.check_stability import assess, parse_rows


class FourCameraStabilityEvidenceTests(unittest.TestCase):
    def setUp(self):
        self.text = (
            "GROUP GRAPH nvstreammux(batch=4, inference=0\n"
        ...
        self.sources = {"CAM-01": [], "CAM-02": [], "CAM-03": [], "CAM-04": []}
        self.group = []
        self.gpu = []
    ...
    def test_missing_camera_rejected(self):
        data = {"CAM-01": self.sources["CAM-01"], "CAM-02": self.sources["CAM-02"], "CAM-03": self.sources["CAM-03"]}
