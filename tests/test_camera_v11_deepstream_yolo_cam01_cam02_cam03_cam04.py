...
LAUNCHER = ROOT / "scripts/run_camera_v11_deepstream_yolo_cam01_cam02_cam03_cam04_v1.sh"
...
ARCH = (
    "CAMERA_V11_DS_YOLO_MULTI_ARCH cameras=4 camera_ids=CAM-01,CAM-02,CAM-03,CAM-04 "
    "rtsp_sources=4 rtsp_sessions=4 rtsp_per_camera=1 decode=deepstream-nvdec "
...
THREAD = "CAMERA_V11_DS_YOLO_MULTI_DETECTOR_THREAD state=START workers=1 cameras=4 scheduler=round-robin\n"
START = "CAMERA_V11_DS_YOLO_MULTI_START cameras=CAM-01,CAM-02,CAM-03,CAM-04 state=async\n"
...
def body(infer_cam04: float = 1.6) -> str:
...
def test_launcher_locks_exact_four_camera_increment() -> None:
    source = LAUNCHER.read_text()
    assert 'BRANCH_EXPECTED="rebuild/service-architecture-v11-deepstream-yolo-cam01-cam02-cam03-cam04-v1-20260901"' in source
    assert 'CAMERAS="${V11_DS_YOLO_CAMERAS:-CAM-01,CAM-02,CAM-03,CAM-04}"' in source
    assert 'camera_set=$CAMERAS expected=CAM-01,CAM-02,CAM-03,CAM-04' in source
...
def test_checker_accepts_four_fair_cameras(tmp_path: Path) -> None:
    result = run_checker(tmp_path, body())
    assert result.returncode == 0, result.stdout + result.stderr
    assert "RESULT=PASS" in result.stdout


def test_checker_rejects_cam04_starvation(tmp_path: Path) -> None:
    result = run_checker(tmp_path, body(infer_cam04=0.5))
    assert result.returncode == 1
    assert "CAM-04:infer_hz_min=0.50<1.20" in result.stdout
