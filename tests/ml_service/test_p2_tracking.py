...
class P2TrackingEvaluationTests(unittest.TestCase):
    ...
    def test_long_occlusion_is_not_kept_indefinitely(self):
        self.assertEqual(self.report["current"]["B_long_75"]["fragments"],1)
    ...
    def test_nvdcf_capability_is_reported_honestly(self):
        ...
        self.assertIn("primary_inference_supported",self.report["nvdcf"])
        self.assertIn("compute_capability",self.report["nvdcf"])
        if self.report["nvdcf"]["compute_capability"]=="6.1":
            self.assertFalse(self.report["nvdcf"]["primary_inference_supported"])
